    ...
    renderCrafting: function () {
        ...
        const renderRecipeFamily = (fam) => {
            ...
            const lines = fam.map(r => {
                ...
                return `<div class="text-sm" style="${can ? '' : 'opacity:0.6;'}">${reqStr}</div>`;
            });
            const anyCan = bestId !== null;
            if (!anyCan) bestId = fam[0].id; // cíl pro disabled tlačítko

            const reqBlock = lines.join(`<div class="text-sm" style="opacity:0.5; margin:2px 0;">${orLabel}</div>`);
