const Game = {
    ...
    // Conversi output efficiency (25.7.2026 sprint) — roste podle SKUTEČNÉ
    // odvedené práce na daném tabu (k.taskXp[tabId]), ne podle kalendáře/
    // loyalty. Konvrš startuje skoro na 1.0, famulus roste nad standard
    // (ostřílená sezónní ruka), oblát startuje nejslabší. Aplikováno jen
    // na zahony/sad/apiarium (plný efekt) a jako bonus-only na pole/
    // vinohrad/piscina — viz komentáře u jednotlivých volání.
    CONVERSI_EFFICIENCY_CURVE: {
        famulus: { floor: 0.85, cap: 1.15, rate: 0.10 },
        oblat:   { floor: 0.50, cap: 0.90, rate: 0.08 },
        _default: { floor: 0.85, cap: 1.00, rate: 0.15 } // konvrš (k.type null/undefined)
    },

    conversiEfficiency: function(k, tabId) {
        if (!k) return 1.0;
        const cfg = this.CONVERSI_EFFICIENCY_CURVE[k.type] || this.CONVERSI_EFFICIENCY_CURVE._default;
        const xp = (k.taskXp && k.taskXp[tabId]) || 0;
        return cfg.floor + (cfg.cap - cfg.floor) * (1 - Math.exp(-cfg.rate * xp));
    },

    // Volat při KAŽDÉM skutečně odvedeném úkolu (didWork/didHarvest) —
    // zrcadlí dormitoriumAddXp pro bratry, ale pro konvrše/famuly/obláty.
    conversiAddTaskXp: function(k, tabId) {
        if (!k) return;
        if (!k.taskXp) k.taskXp = {};
        k.taskXp[tabId] = (k.taskXp[tabId] || 0) + 1;
    },
    ...
    checkConversiChores: function(onlyTab) {
            ...
            if (Date.now() - GameState.conversiGardenLastTick >= DAY) {
                ...
                if (didWork) {
                    if (gardener) {
                        ...
                        this.conversiAddTaskXp(gardener, 'zahony');
                    }
                    if (gardenBrother) {
        ...
        if ((!onlyTab || onlyTab === 'sad') && (orchardKeeper || orchardBrother) && GameState.orchard) {
            ...
            if (Date.now() - GameState.conversiOrchardLastTick >= DAY) {
                ...
                (GameState.orchard || []).forEach(slot => {
                    ...
                    if (!fruit) return;
                    const baseQty = (slot.treeType === 'seed_walnut' || slot.treeType === 'seed_sorb') ? 2 : 3;
                    const qty = Math.max(1, Math.round(baseQty * brotherMult * this.conversiEfficiency(orchardKeeper, 'sad')));
                    this.addItem(fruit, qty);
                ...
                if (didHarvest) {
                    if (orchardKeeper) {
                        ...
                        this.conversiAddTaskXp(orchardKeeper, 'sad');
                    }
                    if (orchardBrother) {
        ...
        if ((!onlyTab || onlyTab === 'apiarium') && (beekeeper || apiaryBrother) && GameState.apiary && typeof GardenSystem !== 'undefined') {
            ...
            if (Date.now() - GameState.conversiApiaryLastTick >= DAY) {
                ...
                GameState.apiary.forEach(hive => {
                    ...
                    const honeyBase = { spring: 1, summer: 3, autumn: 1 };
                    const waxBase   = { spring: 1, summer: 1, autumn: 2 };
                    const konvrsEff = this.conversiEfficiency(beekeeper, 'apiarium');
                    const hQty = Math.max(1, Math.round(honeyBase[season] * strengthMod * queenMod * weatherMod * varroaPenalty * brotherMult * konvrsEff));
                    const wQty = Math.max(1, Math.round(waxBase[season] * strengthMod * varroaPenalty * brotherMult * konvrsEff));
                ...
                if (didWork) {
                    if (beekeeper) {
                        ...
                        this.conversiAddTaskXp(beekeeper, 'apiarium');
                    }
                    if (apiaryBrother) {
        ...
        if ((!onlyTab || onlyTab === 'piscina') && (fisherman || piscinaBrother) && GameState.piscina && GameState.piscina.tier >= 1) {
            ...
            if (Date.now() - GameState.conversiPiscinaLastTick >= DAY) {
                ...
                const carpTotal = (p.fish || []).filter(r => r.stage === 'adult' && r.species === 'kapr').reduce((s, r) => s + r.qty, 0);
                if (carpTotal > 0) {
                    const qty = Math.max(carpTotal, Math.round(carpTotal * brotherMult * this.conversiEfficiency(fisherman, 'piscina')));
                    (p.fish || []).forEach(r => { if (r.stage === 'adult' && r.species === 'kapr') r.qty = 0; });
                    this.addItem('carp', qty);
                ...
                if (didWork) {
                    if (fisherman) {
                        ...
                        this.conversiAddTaskXp(fisherman, 'piscina');
                    }
                    if (piscinaBrother) {
            ...
            if (Date.now() - GameState.conversiFieldLastTick >= DAY) {
                ...
                GameState.fields.forEach((field, idx) => {
                    ...
                    if (field.phase >= 3) {
                        ...
                        Object.keys(GameState.inventory).forEach(itemId => {
                            ...
                            if (gained <= 0) return;
                            const combinedMult = brotherMult * this.conversiEfficiency(plowman, 'pole');
                            if (combinedMult > 1.0) {
                ...
                if (didWork) {
                    if (plowman) {
                        ...
                        this.conversiAddTaskXp(plowman, 'pole');
                    }
                    if (fieldBrother) {
            ...
            if (Date.now() - GameState.conversiVineaLastTick >= DAY) {
                ...
                GameState.vinea.forEach((slot, idx) => {
                    ...
                    if (slot.state === 'ripe') {
                        ...
                        Object.keys(GameState.inventory).forEach(itemId => {
                            let gained = (GameState.inventory[itemId] || 0) - (before[itemId] || 0);
                            if (gained <= 0) return;
                            const combinedMult = brotherMult * this.conversiEfficiency(vintner, 'vinohrad');
                            if (combinedMult > 1.0) {
                                const bonus = Math.round(gained * (combinedMult - 1.0));
                                if (bonus > 0) { this.addItem(itemId, bonus); gained += bonus; }
                            }
                ...
                if (didWork) {
                    if (vintner) {
                        ...
                        this.conversiAddTaskXp(vintner, 'vinohrad');
                    }
                    if (vineaBrother) {
