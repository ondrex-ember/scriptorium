const Game = {
    ...
    load: function() {
        ...
        function applyData(rawData, source) {
            try {
                const loadedState = typeof rawData === 'string' ? JSON.parse(rawData) : rawData;
                deepMerge(GameState, loadedState);
                console.log('✅ Save loaded from ' + source);
                Game.syncTechUnlocks();
            } catch(e) {
                console.error('❌ Load error (' + source + '):', e);
            }
        }

        // Try IDB first (newer/more reliable on mobile), fall back to localStorage
        Game._idbLoad().then(idbRecord => {
            const lsData = localStorage.getItem('scriptorium_save_v6_4');
            const lsTs = (() => { try { return JSON.parse(lsData || '{}').lastSeen || 0; } catch(e) { return 0; } })();
            const idbTs = idbRecord ? (idbRecord.ts || 0) : 0;

            if (idbRecord && idbTs >= lsTs) {
                applyData(idbRecord.data, 'IndexedDB');
            } else if (lsData) {
                applyData(lsData, 'localStorage');
            } else {
            ...
            try {
                ...
                if (data) applyData(data, 'localStorage (IDB unavailable)');
            } catch(e) {
                console.error('❌ Load error (localStorage fallback):', e);
            }
        });
    },
