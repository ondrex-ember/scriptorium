const Game = {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // TAH VYZ — jarní úlovek. Vyza se neplemení v rybníce (žádný fry/young),
    // chytá se jako dospělá na zátarasu při jarním tahu k výtěru. Zátaras
    // vyloví jednu živou rybu do sádek jako "živou konzervu" (čerstvá zásoba
    // masa) — žádný nový state model, jen sezónní event + inventory item,
    // stejný vzor jako podzimní Vylov výše.
    // ═══════════════════════════════════════════════════════════════════════════

    TAH_VYZ_WAIT_DAYS: 4,

    startTahVyz: function() {
        const p = GameState.piscina;
        const lang = (GameState.settings && GameState.settings.language) || 'cs';
        ...
        if (month !== 3 && month !== 4) {
            UI.notify(lang === 'en' ? '❌ The sturgeon run happens only in spring (Mar–Apr).' : '❌ Vyzí tah probíhá jen na jaře (březen–duben).', true);
            return;
        }
        const DAY = 24 * 3600000;
        GameState.piscinaTahVyz = { active: true, startedAt: Date.now(), readyAt: Date.now() + Game.TAH_VYZ_WAIT_DAYS * DAY, notifiedReady: false };
        Game.save(); UI.renderPiscina();
        UI.notify(lang === 'en' ? '🌊 The weir is set — waiting for the sturgeon run.' : '🌊 Zátaras nastražen — čeká se na vyzí tah.');
    },

    checkTahVyzStatus: function() {
        const v = GameState.piscinaTahVyz;
        if (!v || !v.active || v.notifiedReady) return;
        if (Date.now() >= v.readyAt) {
            v.notifiedReady = true;
            if (typeof NotificationSystem !== 'undefined' && NotificationSystem.panel) {
                const lang = (GameState.settings && GameState.settings.language) || 'cs';
                NotificationSystem.panel('🌊 ' + (lang === 'en'
                    ? 'A sturgeon is caught in the weir, alive — bring it to the holding tank.'
                    : 'V zátarasu uvízla živá vyza — přenes ji do sádek.'), 'info');
            }
            Game.save();
        }
    },

    harvestTahVyz: function() {
        const v = GameState.piscinaTahVyz;
        const lang = (GameState.settings && GameState.settings.language) || 'cs';
        if (!v || !v.active) { UI.notify(t('game.busy'), true); return; }
        if (Date.now() < v.readyAt) { UI.notify(lang === 'en' ? '⏳ Still waiting at the weir.' : '⏳ U zátarasu se ještě čeká.', true); return; }
        this.addItem('vyza_sadky_fresh', 1);
        GameState.piscinaTahVyz = { active: false, startedAt: 0, readyAt: 0, notifiedReady: false };
        Game.save(); UI.renderPiscina();
        UI.notify(lang === 'en' ? '🌊 Sturgeon caught alive → holding tank.' : '🌊 Vyza ulovena živá → sádky.');
    },

    // Zpracování vyzy ze sádek — kdykoliv, hráč sám určí čas (fikčně: "než přijde host").
    harvestVyza: function() {
        const lang = (GameState.settings && GameState.settings.language) || 'cs';
        if ((GameState.inventory['vyza_sadky_fresh'] || 0) < 1) { UI.notify(lang === 'en' ? '❌ No sturgeon in the holding tank.' : '❌ Žádná vyza v sádkách.', true); return; }
        this.removeItem('vyza_sadky_fresh', 1);
        this.addItem('vyza_maso', 1);
        this.addItem('vyza_jikry', 1);
        this.addItem('klih', 2);
        Game.save(); UI.renderPiscina();
        UI.notify(lang === 'en' ? '🐋 Sturgeon processed: meat, roe, isinglass ×2.' : '🐋 Vyza zpracována: maso, jikry, klih ×2.');
    },
    ...
    templumDailyTick: function() {
        ...
        // TODO (příští sprint): tahle fatigue-gate podmínka je identická na
        // 9 místech v souboru (Kostel/Hřbitov/Dvůr/Scavenge/Doly/...), ale
        // žádné z nich neškáluje výstup podle typu konvrše (Oblát pracuje
        // na plný výkon, viz TODO u hireOblat()). Refaktor: sdílený
        // Game.conversiEfficiency(k) hook volaný ze všech 9 míst.
    ...
    // TODO (příští sprint): Oblát dnes pracuje na plný výkon jako konvrš —
    // MRD "málo platný zpočátku" NENÍ implementováno. Vyžaduje refaktor
    // sdíleného output multiplieru napříč 9 task funkcemi (viz TODO u první
    // fatigue-gate podmínky, cleanKostel bloku). Zámerně odloženo, ne dnešní
    // riziko — 9 bespoke zásahů do 5+ subsystémů (Kostel/Hřbitov/Dvůr/atd.).
