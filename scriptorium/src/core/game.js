const Game = {
    ...
    // historicky-podklad-mrd (9.8.2026): 0-1 float, jak daleko jsme uvnitř
    // aktuální sezóny (stejné měsíční hranice jako _getApiarySeason).
    // Sdílený helper — použije ho i budoucí sezónní scavenge gating.
    seasonProgress: function () {
        const d = new Date();
        const season = this._getApiarySeason();
        const BOUNDS = { spring: [2, 3, 4], summer: [5, 6, 7], autumn: [8, 9, 10], winter: [11, 0, 1] }; // 0-indexed getMonth()
        const months = BOUNDS[season];
        const startMonth = months[0];
        let startYear = d.getFullYear();
        // Zima přetéká přes Nový rok (prosinec→leden→únor) — pokud jsme
        // v lednu/únoru, sezóna začala loni v prosinci.
        if (season === 'winter' && d.getMonth() < 11) startYear -= 1;
        const start = new Date(startYear, startMonth, 1).getTime();
        const end = new Date(startYear, startMonth + 3, 1).getTime();
    ...
    giveAlms: function () {
        ...
        this.addItem('candle', Math.round(1 * streakMult));
        if (!GameState.secrets) GameState.secrets = {};
