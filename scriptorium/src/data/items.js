const ItemsDB = {
    ...
    "bark": { name: "Kůra", name_en: "Bark", icon: "🍂", type: "mat", desc: "Kůra stromu.", desc_en: "Tree bark.", seasonCat: "drevo_kura" },
    ...
    "water": { name: "Voda", name_en: "Water", icon: "💧", type: "food", desc: "Voda ze studny nebo z přírody. Ne vždy čistá.", desc_en: "Water from a well or the wild. Not always clean.", seasonCat: "rybolov" },
    ...
    "herb_red": { name: "Krvavý květ", name_en: "Bloodwort", icon: "🌺", type: "mat", desc: "Bylina.", desc_en: "A red healing herb.", seasonCat: "aromaticke_byliny" },
    "fat": { name: "Tuk", name_en: "Fat", icon: "🥩", type: "mat", desc: "Zvířecí tuk.", desc_en: "Animal fat.", seasonCat: "lov" },
    "tallow": { name: "Lůj", name_en: "Tallow", icon: "🫗", type: "mat", desc: "Tvrdý přežvýkavý tuk (jelen, kráva, ovce). Po vychladnutí tuhne na kámen. Na pochodně, svíčky a kůže.", desc_en: "Hard ruminant fat (deer, cattle, sheep). Hardens like stone once cooled. For torches, candles and leather.", seasonCat: "lov" },
    "meat": { name: "Divoké maso", name_en: "Wild Game Meat", icon: "🍖", type: "food_raw", desc: "Syrové maso z ulovené zvěře. K vaření, ne k prodeji na trhu.", desc_en: "Raw meat from hunted game. For cooking, not for the market stall.", seasonCat: "lov" },
    "crayfish": { name: "Raci", name_en: "Crayfish", icon: "🦞", type: "food_raw", desc: "Sladkovodní raci z mokřadu.", desc_en: "Freshwater crayfish from the wetlands.", seasonCat: "mokradni_fauna" },
    "snail": { name: "Hlemýždi", name_en: "Snails", icon: "🐌", type: "food_raw", desc: "Hlemýždi s ulitou. Jiní tvorové než slimák.", desc_en: "Shelled snails. A different creature from the slug.", seasonCat: "mokradni_fauna" },
    "cervec": { name: "Červec", name_en: "Cochineal Grubs", icon: "🐛", type: "mat", desc: "Drobný hmyz sbíraný u kořenů luk kolem svatojánské noci. Rozdrcený dává karmínově rudé barvivo — polský červec, cenný před dovozem mexického košenilu.", desc_en: "Tiny grubs gathered from meadow roots around Midsummer. Crushed, they yield a crimson dye — Polish cochineal, prized before Mexican cochineal was imported.", seasonCat: "hmyz_teplo" },
    ...
    "bone": { name: "Kost", name_en: "Bone", icon: "☠️", type: "mat", desc: "Tvrdá kost.", desc_en: "Hard bone.", seasonCat: "lov" },
    ...
    "torch_tallow": { name: "Louč (lojová)", name_en: "Torch (tallow)", icon: "🪵", type: "tool", tier: "fire", lightHours: 3, desc: "Louč z klacku napuštěného lojem. Lůj po vychladnutí ztuhne a hoří stabilně a jasně, bez velkého kapání. 3h světla.", desc_en: "A torch soaked in tallow. Tallow hardens as it cools and burns stable and bright, without much dripping. 3h of light." },
    "torch_resin": { name: "Louč (smolná)", name_en: "Torch (resin)", icon: "🔥", type: "tool", tier: "fire", lightHours: 6, desc: "Louč napuštěná pryskyřicí jehličnanů. Hoří jasně, stabilně a odolá i větru. 6h světla.", desc_en: "A torch soaked in conifer resin. Burns bright, stable, and holds up against wind. 6h of light." },
    "candle": { name: "Svíčka (tuková)", name_en: "Candle (fat)", icon: "🕯️", type: "tool", tier: "fire", lightHours: 4, desc: "Ze zvířecího tuku. Čadí a páchne jako přepálený omastek, plamen skomírá — nejhorší dostupná svíce. 4h světla.", desc_en: "Made of animal fat. Smokes and reeks of scorched grease, the flame gutters — the poorest candle available. 4h of light." },
    // svitidla-mrd (16.8.2026) — lůj a vosk existovaly jako itemy dřív
    // (tallow, beeswax), ale žádnej recept je nikdy nepoužil na svíčku.
    // 'candle' id zůstává netknutý (Mše/pohřby/svatby/dary/Chronicon —
    // 15+ míst) — tyhle dva jsou nový, samostatný tiery vedle, ne náhrada.
    "candle_tallow": { name: "Svíčka (lojová)", name_en: "Candle (tallow)", icon: "🕯️", type: "tool", tier: "fire", lightHours: 8, desc: "Z hovězího či skopového loje. Standard obyčejných lidí — hoří stabilně, bez velkého čadění, na rozdíl od tukové.", desc_en: "Made of beef or mutton tallow. The common standard — burns steadily, without heavy smoke, unlike the fat candle." },
    "candle_wax": { name: "Voskavka", name_en: "Beeswax Candle", icon: "🕯️", type: "tool", tier: "fire", lightHours: 14, desc: "Z čistého včelího vosku. Jasný, stabilní plamen, téměř bez kouře a s příjemnou vůní — jediná svíce hodná oltáře a skriptoria.", desc_en: "Pure beeswax. A bright, steady flame, almost smokeless and sweetly scented — the only candle fit for the altar and the scriptorium." },
    ...
    "kozene_boty": { name: "Kožené boty", name_en: "Leather Shoes", icon: "👢", type: "mat", desc: "Obyčejné kožené boty od městského ševce. Pohodlnější než dřeváky, dražší.", desc_en: "Plain leather shoes from a town cobbler. More comfortable than clogs, costlier." },
    ...
    "britva_chirurgus": {
        name: "Břitva ranhojiče", name_en: "The Surgeon's Razor", icon: "🪒", type: "tool",
    ...
    "seeds_herb": { name: "Semínka", name_en: "Seeds", icon: "🌱", type: "mat", desc: "Rostliny.", desc_en: "Plant seeds.", seasonCat: "aromaticke_byliny" },
    ...
    // ALCHEMY INGREDIENTS
    "frog": { name: "Žába", name_en: "Frog", icon: "🐸", type: "alchemy_ing", desc: "Z mokřadu.", desc_en: "From the wetlands.", seasonCat: "mokradni_fauna" },
    "slug": { name: "Slimák", name_en: "Slug", icon: "🐌", type: "alchemy_ing", desc: "Sliz.", desc_en: "Slimy creature.", seasonCat: "mokradni_fauna" },
    "resin": { name: "Pryskyřice", name_en: "Resin", icon: "💧", type: "alchemy_ing", desc: "Ze stromů.", desc_en: "Tree resin.", seasonCat: "drevo_kura" },
    ...
    "honey": { name: "Med", name_en: "Honey", icon: "🍯", type: "food", desc: "Včelí med.", desc_en: "Bee honey.", seasonCat: "hmyz_teplo" },
    "mushroom_poison": { name: "Muchotrávka", name_en: "Death Cap", icon: "🍄", type: "alchemy_ing", desc: "Jedovatá houba.", desc_en: "A poisonous mushroom.", seasonCat: "podzimni_houby" },
    "roots": { name: "Kořeny", name_en: "Roots", icon: "🪴", type: "alchemy_ing", desc: "Hluboké kořeny.", desc_en: "Deep roots.", seasonCat: "koreny_hlizy" },
    "nightshade": { name: "Rulík", name_en: "Nightshade", icon: "🖤", type: "alchemy_ing", desc: "Jedovatý, léčivý.", desc_en: "Deadly yet medicinal.", seasonCat: "jedovate_rostliny" },

    // FOOD RAW
    "fish": { name: "Ryba", name_en: "Fish", icon: "🐟", type: "food_raw", desc: "Čerstvá ryba.", desc_en: "Fresh fish." },
    "mushroom": { name: "Houby", name_en: "Mushrooms", icon: "🍄", type: "food", desc: "Jedlé houby.", desc_en: "Edible mushrooms.", seasonCat: "podzimni_houby" },
    ...
    "leek": { name: "Pór", name_en: "Leek", icon: "🌿", type: "food_raw", desc: "Allium porrum. Klášterní zahrada ze Sankt Gallenu. Základ polévky.", desc_en: "Allium porrum. From the St Gallen monastery plan. Soup staple.", seasonCat: "zimuvzdorna_zelenina" },
    ...
    "garlic": { name: "Česnek", name_en: "Garlic", icon: "🧄", type: "food_raw", desc: "Allium sativum. Lék i koření. Hildegarda: teplý a suchý, zahání nemoci.", desc_en: "Allium sativum. Medicine and spice. Hildegard: warm and dry, drives off sickness.", seasonCat: "aromaticke_byliny" },
    "ground_elder": { name: "Bršlice kozí noha", name_en: "Ground Elder", icon: "🌿", type: "food_raw", desc: "Aegopodium podagraria. Dnes plevel, ve středověku hlavní jarní zelenina.", desc_en: "Aegopodium podagraria. A weed today, a staple spring green in the Middle Ages.", seasonCat: "jarni_zelenina" },
    "goosefoot": { name: "Lebeda", name_en: "Goosefoot", icon: "🌿", type: "food_raw", desc: "Chenopodium album. Rostla kolem obydlí, listy bohaté na bílkoviny.", desc_en: "Chenopodium album. Grew around dwellings, leaves rich in protein.", seasonCat: "jarni_zelenina" },
    "sorrel": { name: "Šťovík", name_en: "Sorrel", icon: "🌿", type: "food_raw", desc: "Rumex acetosa. Kyselá chuť tam, kde chyběl citron i ocet.", desc_en: "Rumex acetosa. A sour bite where lemon and vinegar were scarce.", seasonCat: "jarni_zelenina" },
    "dandelion": { name: "Pampeliška", name_en: "Dandelion", icon: "🌼", type: "food_raw", desc: "Taraxacum. Jarní očistná bylina, i s ptačincem žabincem.", desc_en: "Taraxacum. A spring cleansing herb, alongside chickweed.", seasonCat: "jarni_zelenina" },
    "burdock_root": { name: "Kořen lopuchu", name_en: "Burdock Root", icon: "🥕", type: "food_raw", desc: "Arctium lappa. Hladové jídlo — kořen pečený v popelu.", desc_en: "Arctium lappa. Famine food — root baked in ashes.", seasonCat: "koreny_hlizy" },
    "couch_grass": { name: "Pýr plazivý", name_en: "Couch Grass", icon: "🌾", type: "food_raw", desc: "Elymus repens. Oddenky plné škrobu, mlely se na mouku.", desc_en: "Elymus repens. Starch-rich rhizomes, ground into flour.", seasonCat: "koreny_hlizy" },
    "cattail_root": { name: "Kořen orobince", name_en: "Cattail Root", icon: "🌾", type: "food_raw", desc: "Typha. Rostl kolem rybníků, oddenky bohaté na škrob.", desc_en: "Typha. Grew around ponds, starch-rich rhizomes.", seasonCat: "koreny_hlizy" },
    "potato": { name: "Brambora", name_en: "Potato", icon: "🥔", type: "food_raw", desc: "Ze zahrady.", desc_en: "From the garden." },
    "berries": { name: "Bobule", name_en: "Berries", icon: "🫐", type: "food", hunger: 2, desc: "Lesní plody. Lze jíst syrové — zasytí na 2h.", desc_en: "Forest berries. Edible raw — fills for 2h.", seasonCat: "letni_bobule" },
    "rosehip": { name: "Šípky", name_en: "Rosehips", icon: "🔴", type: "food_raw", desc: "Fructus rosae. Hlavní zdroj vitamínů na zimu.", desc_en: "Fructus rosae. The main source of winter vitamins.", seasonCat: "podzimni_plane_ovoce" },
    "beechnut": { name: "Bukvice", name_en: "Beechnuts", icon: "🌰", type: "food_raw", desc: "Fagus sylvatica. Z bukvic se lisoval i jedlý olej.", desc_en: "Fagus sylvatica. Beechnuts were also pressed for edible oil.", seasonCat: "podzimni_orechy" },
    "wild_fruit": { name: "Planá jablka a hrušky", name_en: "Wild Apples and Pears", icon: "🍏", type: "food_raw", desc: "Pláňata. Tvrdá a trpká, syrová se nedala jíst.", desc_en: "Wild fruit. Hard and bitter, inedible raw.", seasonCat: "podzimni_plane_ovoce" },
    "cornel_cherry": { name: "Dřínky", name_en: "Cornel Cherries", icon: "🍒", type: "food_raw", desc: "Cornus mas. Kyselé plody dřínu obecného.", desc_en: "Cornus mas. Tart fruit of the cornelian cherry.", seasonCat: "podzimni_plane_ovoce" },
    "sloe": { name: "Trnky", name_en: "Sloes", icon: "🫐", type: "food_raw", desc: "Prunus spinosa. Po prvním mrazu ztrácí svíravou trpkost.", desc_en: "Prunus spinosa. The first frost takes away their sharp bitterness.", seasonCat: "podzimni_plane_ovoce" },
    "morel": { name: "Smrže", name_en: "Morels", icon: "🍄", type: "food_raw", desc: "Vzácná a ceněná jarní houba.", desc_en: "A rare and prized spring mushroom.", seasonCat: "jarni_houba" },
    "saffron_milk_cap": { name: "Ryzce", name_en: "Saffron Milk Caps", icon: "🍊", type: "food_raw", desc: "Nakládají se do slaného nálevu — vydrží celou zimu.", desc_en: "Preserved in brine — they keep all winter.", seasonCat: "podzimni_houby" },
    "porcini": { name: "Hřiby", name_en: "Porcini", icon: "🍄", type: "food_raw", desc: "Nejceněnější houba lesa.", desc_en: "The most prized mushroom of the forest.", seasonCat: "podzimni_houby" },
    "bracket_fungus": { name: "Choroš sírový", name_en: "Chicken of the Woods", icon: "🍊", type: "food_raw", desc: "Roste na stromech, ne na zemi. Struktura podobná kuřecímu masu.", desc_en: "Grows on trees, not the ground. Texture similar to chicken.", seasonCat: "podzimni_houby" },
    ...
    "berry_jam": { name: "Bobulová povidla", name_en: "Berry Jam", icon: "🫐", type: "food", hunger: 5, desc: "Bobule svařené s medem v Athanoru. Bez cukru, jen med a čas.", desc_en: "Berries boiled down with honey in the Athanor. No sugar — just honey and time." },
    "apple_jam": { name: "Jablečná povidla", name_en: "Apple Jam", icon: "🍎", type: "food", hunger: 5, desc: "Sadová jablka svařená s medem do husté hmoty.", desc_en: "Orchard apples boiled down with honey into a thick paste." },
    "quince_marmalade": { name: "Kdoulová marmeláda", name_en: "Quince Marmalade", icon: "🍋", type: "food", hunger: 6, desc: "Marmelada — slovo pochází z portugalského marmelo, kdoule. Původní a jediná pravá marmeláda.", desc_en: "Marmalade — the word itself comes from Portuguese marmelo, quince. The original, and only true, marmalade." },
    "plum_jam": { name: "Švestková povidla", name_en: "Plum Jam", icon: "🫐", type: "food", hunger: 5, desc: "Švestky ze sadu, dlouze vařené s medem.", desc_en: "Orchard plums, long-boiled with honey." },
    "cornel_cherry_jam": { name: "Dřínková povidla", name_en: "Cornel Cherry Jam", icon: "🍒", type: "food", hunger: 5, desc: "Trpké dřínky zkrocené medem a dlouhým varem.", desc_en: "Tart cornel cherries tamed with honey and long boiling." },
    "wild_fruit_jam": { name: "Povidla z planého ovoce", name_en: "Wild Fruit Jam", icon: "🍏", type: "food", hunger: 5, desc: "Planá jablka a hrušky, jinak nejedlá syrová, svařená s medem.", desc_en: "Wild apples and pears, inedible raw otherwise, boiled down with honey." },
    "spiced_marmalade": { name: "Kořeněná marmeláda", name_en: "Spiced Monastery Marmalade", icon: "✨", type: "food", hunger: 7, desc: "Kdoule, med a skořice — hodná daru opatovi nebo prodeje za zlaté.", desc_en: "Quince, honey and cinnamon — worthy of a gift to the abbot, or sale for gold." },
    ...
    // HERBS & SEEDS
    "herb_yellow": { name: "Heřmánek", name_en: "Chamomile", icon: "🌼", type: "mat", desc: "Uklidňující bylina.", desc_en: "A calming herb.", seasonCat: "aromaticke_byliny" },
    "herb_blue": { name: "Levandule", name_en: "Lavender", icon: "💜", type: "mat", desc: "Na spaní.", desc_en: "For sleep.", seasonCat: "aromaticke_byliny" },
    "mint": { name: "Máta", name_en: "Mint", icon: "🌿", type: "mat", desc: "Osvěžující.", desc_en: "Refreshing.", seasonCat: "aromaticke_byliny" },
    "seeds_vegetable": { name: "Semínka zeleniny", name_en: "Vegetable Seeds", icon: "🌱", type: "mat", desc: "Zelenina.", desc_en: "Vegetable seeds.", seasonCat: "semena_letni_podzim" },
    "seeds_yellow": { name: "Semínka heřmánku", name_en: "Chamomile Seeds", icon: "🌾", type: "mat", desc: "Žlutá bylina.", desc_en: "Yellow herb seeds.", seasonCat: "aromaticke_byliny" },
    "seeds_blue": { name: "Semínka levandule", name_en: "Lavender Seeds", icon: "🌾", type: "mat", desc: "Modrá bylina.", desc_en: "Blue herb seeds.", seasonCat: "aromaticke_byliny" },
    "seeds_mint": { name: "Semínka máty", name_en: "Mint Seeds", icon: "🌾", type: "mat", desc: "Máta.", desc_en: "Mint seeds.", seasonCat: "aromaticke_byliny" },
    "seeds_thyme": { name: "Semínka tymiánu", name_en: "Thyme Seeds", icon: "🌾", type: "mat", desc: "Tymián. Léčí Varroa, koření jídla.", desc_en: "Thyme. Treats Varroa, seasons food.", seasonCat: "semena_letni_podzim" },
    "seeds_kopr": { name: "Semínka kopru", name_en: "Dill Seeds", icon: "🌾", type: "mat", desc: "Kopr. Vzácnější semínko, k rakům se hodí nejlíp.", desc_en: "Dill. A rarer seed, pairs best with crayfish.", seasonCat: "semena_letni_podzim" },
    "seeds_sage": { name: "Semínka šalvěje", name_en: "Sage Seeds", icon: "🌾", type: "mat", desc: "Šalvěj. Salvia — zachraňuje.", desc_en: "Sage. Salvia — it saves.", seasonCat: "semena_letni_podzim" },
    "seeds_fennel": { name: "Semínka fenyklu", name_en: "Fennel Seeds", icon: "🌾", type: "mat", desc: "Fenykl. Na trávení.", desc_en: "Fennel. For digestion." },
    "seeds_wormwood": { name: "Semínka pelynku", name_en: "Wormwood Seeds", icon: "🌾", type: "mat", desc: "Pelyněk. Hořký jako pokání.", desc_en: "Wormwood. Bitter as penance.", seasonCat: "semena_letni_podzim" },
    ...
    "seeds_yarrow": { name: "Semínka řebříčku", name_en: "Yarrow Seeds", icon: "🌾", type: "mat", desc: "Řebříček. Hojení ran.", desc_en: "Yarrow. Wound healing.", seasonCat: "semena_letni_podzim" },
    ...
    "seeds_garlic": { name: "Stroužky česneku", name_en: "Garlic Cloves", icon: "🧄", type: "mat", desc: "Česnek. Sází se stroužky, ne semeny.", desc_en: "Garlic. Planted as cloves, not seeds.", seasonCat: "semena_letni_podzim" },
    "seeds_rye": { name: "Osivo žita", name_en: "Rye Seed", icon: "🌾", type: "mat", desc: "Osivo žita. Ozimá plodina pro pole.", desc_en: "Rye seed. A winter crop for the fields.", seasonCat: "semena_letni_podzim" },
    "seeds_wheat": { name: "Osivo pšenice", name_en: "Wheat Seed", icon: "🌾", type: "mat", desc: "Osivo pšenice. Jarní plodina pro pole.", desc_en: "Wheat seed. A spring crop for the fields.", seasonCat: "semena_letni_podzim" },
    "seeds_barley": { name: "Osivo ječmene", name_en: "Barley Seed", icon: "🌾", type: "mat", desc: "Osivo ječmene. Pro pole, i pro pivovar.", desc_en: "Barley seed. For the fields — and the brewery.", seasonCat: "semena_letni_podzim" },
    "seeds_oats": { name: "Osivo ovsa", name_en: "Oat Seed", icon: "🌾", type: "mat", desc: "Osivo ovsa. Krmivo pro koně a dobytek.", desc_en: "Oat seed. Feed for horses and livestock.", seasonCat: "semena_letni_podzim" },
    "seeds_millet": { name: "Osivo prosa", name_en: "Millet Seed", icon: "🌾", type: "mat", desc: "Osivo prosa. Nenáročná plodina pro pole.", desc_en: "Millet seed. An undemanding field crop.", seasonCat: "semena_letni_podzim" },
    ...
    "seeds_peas": { name: "Osivo hrachu", name_en: "Pea Seed", icon: "🌱", type: "mat", desc: "Osivo hrachu. Luštěnina pro pole.", desc_en: "Pea seed. A legume for the fields.", seasonCat: "semena_letni_podzim" },
    ...
    "seeds_flax": { name: "Osivo lnu", name_en: "Flax Seed", icon: "🌱", type: "mat", desc: "Osivo lnu. Přadná plodina, vzácnější.", desc_en: "Flax seed. A fibre crop, rarer to find.", seasonCat: "semena_letni_podzim" },
    "seeds_mandrake": { name: "Semínka mandragory", name_en: "Mandrake Seeds", icon: "🌾", type: "mat", desc: "Vzácná. Opatřit není snadné.", desc_en: "Rare. Not easy to obtain.", seasonCat: "semena_letni_podzim" },
    "seeds_belladonna": { name: "Semínka rulíku", name_en: "Belladonna Seeds", icon: "🌾", type: "mat", desc: "Jedovatá. Zacházet opatrně.", desc_en: "Poisonous. Handle with care.", seasonCat: "semena_letni_podzim" },
    "seeds_poppy": { name: "Semínka máku", name_en: "Poppy Seeds", icon: "🌾", type: "mat", desc: "Mák. Léčivý i jedlý.", desc_en: "Poppy. Medicinal and edible.", seasonCat: "semena_letni_podzim" },
    "seeds_nettle": { name: "Semínka kopřivy", name_en: "Nettle Seeds", icon: "🌱", type: "mat", desc: "Kopřiva. Roste všude, ale v zahradě lépe.", desc_en: "Nettle. Grows anywhere, better in a garden.", seasonCat: "semena_letni_podzim" },
    ...
    // VELLUM CHAIN
    "hide": { name: "Divoká kůže", name_en: "Wild Hide", icon: "🦌", type: "mat", desc: "Nezpracovaná, neidentifikovaná kůže z lovu. Hrubě na useň, nebo pracně na surovou kůži pro pergamen.", desc_en: "Unprocessed, unidentified hide from the hunt. Crudely into rawhide, or laboriously into raw hide for vellum.", seasonCat: "lov" },
    ...
    // QUILL
    "feather": { name: "Husí pero", name_en: "Goose Feather", icon: "🪶", type: "mat", desc: "Z křídla husy.", desc_en: "From a goose wing.", seasonCat: "lov" },
    ...
    "vrbova_kura": { name: "Vrbová kůra", name_en: "Willow Bark", icon: "🌿", type: "alchemy_ing", desc: "Chladná a vlhká kůra vrby. Tiší horečku a bolest.", desc_en: "Cool, moist willow bark. Eases fever and pain.", seasonCat: "drevo_kura" },
    "oak_bark": { name: "Dubová kůra", name_en: "Oak Bark", icon: "🌳", type: "alchemy_ing", desc: "Třísloviny z dubové kůry. Základ pro fixaci barviv i výrobu inkoustu bez kovů.", desc_en: "Tannins from oak bark. A base for fixing dyes and making ink without metals.", seasonCat: "drevo_kura" },
    ...
    // fishing-species-mrd (10.8.2026): 5 nových pokladů z řeky/rybníka,
    // mirror torn_page/wax_seal vzoru. Ne "rezavý kotlík" — to je oddělený,
    // specifický mechanismus (kotlík quest), tady jde o obecné říční nálezy.
    "old_boot": {
        name: "Starý střevíc", name_en: "Old Boot", icon: "👢", type: "mat", lostItem: true,
        desc: "Rozmoklý střevíc vytažený z bahna. Kdo ho tu ztratil a proč jen jeden?", desc_en: "A sodden boot pulled from the mud. Who lost it here, and why only one?"
    },
    "broken_pot_shard": {
        name: "Střep starého hrnce", name_en: "Broken Pot Shard", icon: "🏺", type: "mat", lostItem: true,
        desc: "Kus rozbité nádoby, hladce ohlazený vodou. Léta ležel na dně.", desc_en: "A shard of a broken vessel, smoothed by years underwater."
    },
    "drowned_coin": {
        name: "Utopená mince", name_en: "Drowned Coin", icon: "🪙", type: "mat", lostItem: true,
        desc: "Zčernalá mince vytažená z bahna. Ražba nečitelná, ale je to stříbro.", desc_en: "A blackened coin pulled from the silt. The mint mark illegible, but it is silver."
    },
    "lost_fishing_hook": {
        name: "Ztracený rybářský háček", name_en: "Lost Fishing Hook", icon: "🪝", type: "mat", lostItem: true,
        desc: "Zrezlý háček, dávno utržený z cizí šňůry. Ještě se dá nabrousit.", desc_en: "A rusted hook, long since snapped from someone else's line. It can still be sharpened."
    },
    "water_smoothed_amulet": {
        name: "Vodou ohlazený amulet", name_en: "Water-Smoothed Amulet", icon: "🔮", type: "mat", lostItem: true,
        desc: "Malý přívěsek neznámého tvaru, obroušený proudem do hladka. Pohanský, nebo jen starý?", desc_en: "A small pendant of unclear shape, worn smooth by the current. Pagan, or merely old?"
    },
    "wax_seal": {
    ...
    // ── ZAKÁZKOVÉ KURIOZITY (vrstva 3, zakazky-rozsireni-ctyri-kategorie-mrd.md §6) ──
    "drahokam_ulomek": {
        name: "Úlomek drahokamu", name_en: "Gem Fragment", icon: "💎", type: "mat", lostItem: true,
        desc: "Odštěpek z klenotníkovy dílny — zbytek po broušení, na šperk už malý, na vzpomínku právě dost.", desc_en: "A chip from the goldsmith's workshop — too small for jewellery, just right for a keepsake."
    },
    "glosa_studenta": {
        name: "Glosa neznámého studenta", name_en: "Gloss of an Unknown Student", icon: "✒️", type: "lore", lostItem: true,
        desc: "Okraj vypůjčeného textu popsaný cizí rukou — poznámka, hádka s autorem, možná i vtip. Pecia systém nechává stopy.", desc_en: "The margin of a borrowed text, scrawled by another hand — a note, an argument with the author, perhaps a joke. The pecia system leaves its traces."
    },
    "erbovni_nakres": {
        name: "Nedokončený erbovní nákres", name_en: "Unfinished Heraldic Sketch", icon: "🎨", type: "lore", lostItem: true,
        desc: "Pracovní návrh erbu, co se do finální listiny nakonec nedostal. Štít zůstal napůl vybarvený.", desc_en: "A working sketch of a coat of arms that never made it into the final charter. The shield stayed half-coloured."
    },

    // ATHANOR — ingredience (nové suroviny)
    "carbon_black": { name: "Saze", name_en: "Carbon Black", icon: "🖤", type: "alchemy_ing", desc: "Saze z krbu. Nejstarší černý pigment.", desc_en: "Soot from the hearth. The oldest black pigment.", seasonCat: "nerosty" },
    "ochre": { name: "Okr", name_en: "Ochre", icon: "🟤", type: "alchemy_ing", desc: "Žlutohnědá zemina. Pigment od pravěku.", desc_en: "Yellow-brown earth. A pigment since prehistory.", seasonCat: "nerosty" },
    ...
    "karmin": { name: "Karmín", name_en: "Cochineal Dye", icon: "🔴", type: "alchemy_ing", desc: "Karmínově rudý pigment z drceného červce. Vzácná barva pro iluminace a purpurové iniciály.", desc_en: "A crimson pigment from crushed cochineal grubs. A rare colour for illuminations and purple initials." },
    ...
    // metallurgia-rara-mrd (21.9.2026) — nová vzácná ruda, tech_metallurgia_rara.
    "antimony": { name: "Antimon", name_en: "Antimony", icon: "⬛", type: "alchemy_ing", desc: "Stibium. Těžký, lesklý šedočerný minerál z hlubších štol. Alchymisté mu říkají 'vlk kovů' — čistí zlato od nečistot. Tiskaři jej budou potřebovat do slitiny na litery.", desc_en: "Stibium. A heavy, lustrous grey-black mineral from deeper shafts. Alchemists call it the 'wolf of metals' — it purifies gold of impurities. Printers will need it for the type-metal alloy." },
    ...
    // metallurgia-rara-mrd (21.9.2026) — Knihtisk, Folio 7 (mirror bronz/slitina_cin_olovo).
    "tiskarska_slitina": { name: "Tiskařská slitina", name_en: "Type-Metal Alloy", icon: "🔠", type: "alchemy_ing", desc: "Typometallum. Olovo, cín a antimon tavené dohromady. Při chladnutí se nepatrně rozpíná, takže dokonale vyplní formu litery — a je dost tvrdá, aby vydržela tisíce tlaků lisu.", desc_en: "Typometallum. Lead, tin and antimony melted together. It expands slightly on cooling, filling the letter mold perfectly — and is hard enough to survive thousands of impressions from the press." },
    ...
    "chamomile": { name: "Heřmánek", name_en: "Chamomile", icon: "🌼", type: "herb", desc: "Matka bylinek. Hildegarda ho doporučovala.", desc_en: "Mother of herbs. Hildegard recommended it.", seasonCat: "aromaticke_byliny" },
    "st_johns_wort": { name: "Třezalka", name_en: "St. John's Wort", icon: "🌻", type: "herb", desc: "Bylina sv. Jana. Léčí rány i melancholii.", desc_en: "Herb of St. John. Heals wounds and melancholy.", seasonCat: "aromaticke_byliny" },
    "thyme": { name: "Tymián", name_en: "Thyme", icon: "🌿", type: "herb", desc: "Odvání Varroa z úlů. Hildegarda jej znala dobře.", desc_en: "Drives Varroa from hives. Hildegard knew it well.", seasonCat: "aromaticke_byliny" },
    "sage": { name: "Šalvěj", name_en: "Sage", icon: "🌿", type: "herb", desc: "Salvia officinalis. Kapitulář Karla Velikého. Čistí vzduch i mysl.", desc_en: "Salvia officinalis. Capitulare de villis. Purifies air and mind.", seasonCat: "aromaticke_byliny" },
    "fennel": { name: "Fenykl", name_en: "Fennel", icon: "🌿", type: "herb", desc: "Foeniculum vulgare. Dobrý na trávení, Hildegarda jej doporučovala.", desc_en: "Foeniculum vulgare. Good for digestion, praised by Hildegard.", seasonCat: "aromaticke_byliny" },
    "kopr": { name: "Kopr", name_en: "Dill", icon: "🌿", type: "herb", desc: "Anethum graveolens. Uveden v Kapitulári Karla Velikého mezi povinnými zahradními bylinami.", desc_en: "Anethum graveolens. Listed among the mandatory garden herbs in Charlemagne's Capitulare de villis.", seasonCat: "aromaticke_byliny" },
    "wormwood": { name: "Pelyněk", name_en: "Wormwood", icon: "🌿", type: "herb", desc: "Artemisia absinthium. Bylina všech bylin. Chrání před morem.", desc_en: "Artemisia absinthium. Herb of herbs. Wards off pestilence.", seasonCat: "aromaticke_byliny" },
    "comfrey": { name: "Kostival lékařský", name_en: "Comfrey", icon: "🌿", type: "herb", desc: "Symphytum officinale. Základ středověké ortopedie — obklady na bolavé klouby a kosti.", desc_en: "Symphytum officinale. The foundation of medieval orthopaedics — poultices for aching joints and bones.", seasonCat: "aromaticke_byliny" },
    "juniper": { name: "Jalovec", name_en: "Juniper", icon: "🌲", type: "herb", desc: "Juniperus communis. Silně prohřívací silice, oblíbená na revma.", desc_en: "Juniperus communis. Strongly warming resin, favoured against rheumatism.", seasonCat: "jalovec" },
    "rosemary": { name: "Rozmarýn", name_en: "Rosemary", icon: "🌿", type: "herb", desc: "Rosmarinus officinalis. Prohřívá a stimuluje krevní oběh ve ztuhlé ruce.", desc_en: "Rosmarinus officinalis. Warms and stimulates blood flow in a stiff hand.", seasonCat: "rozmaryn" },
    "hyssop": { name: "Yzop", name_en: "Hyssop", icon: "🌿", type: "herb", desc: "Hyssopus officinalis. Benediktinský klášter bez yzopu? Nemyslitelné.", desc_en: "Hyssopus officinalis. A Benedictine monastery without hyssop? Unthinkable.", seasonCat: "aromaticke_byliny" },
    "yarrow": { name: "Řebříček", name_en: "Yarrow", icon: "🌿", type: "herb", desc: "Achillea millefolium. Hojí rány. Místní česká bylina, v Čechách od nepaměti.", desc_en: "Achillea millefolium. Heals wounds. A native Czech herb, grown here since time immemorial.", seasonCat: "aromaticke_byliny" },
    "plantain": { name: "Jitrocel", name_en: "Plantain", icon: "🌿", type: "herb", desc: "Plantago major. Poutníkova bylina — roste u každé cesty, hojí rány a odvary tiší kašel.", desc_en: "Plantago major. The wayfarer's herb — grows by every road, heals wounds, and its brew soothes coughs.", seasonCat: "aromaticke_byliny" },
    ...
    "nettle": { name: "Kopřiva", name_en: "Nettle", icon: "🌿", type: "special", desc: "Urtica dioica. Vlákno, jídlo i lék. Klášterní zahrady ji pěstovaly záměrně — na látku i odvar.", desc_en: "Urtica dioica. Fibre, food and medicine. Monastery gardens cultivated it deliberately.", seasonCat: "jarni_zelenina" },
    ...
    "potion_vigor_major": { name: "Brtnické tonikum", name_en: "Beeman's Tonic", icon: "🍯", type: "potion", desc: "Včelí chléb louhovaný v medu. Obnoví síly. Vigor +35.", desc_en: "Bee bread steeped in honey. Restores strength. Vigor +35." },
    ...
    "scythe": { maxUses: 400, maxStack: 1, name: "Kosa", name_en: "Scythe", icon: "🌾", type: "tool", tier: "iron", desc: "Kovaná kosa na obilí a trávu od kovářského cechu. Bez ní žne jen srp — pomaleji a s bolavými zády.", desc_en: "A forged scythe for grain and grass from the smiths' guild. Without it there is only the sickle — slower, and harder on the back." },
    ...
    "whetstone_rock": { name: "Brusný kámen", name_en: "Whetstone Rock", icon: "🪨", type: "mat", desc: "Kus jemnozrnné horniny z hlubší štoly — křemičitá břidlice nebo pískovec, vhodný na výrobu brousků.", desc_en: "A piece of fine-grained rock from a deeper shaft — quartz-bearing schist or sandstone, suitable for making whetstones." },
    "whetstone": { maxUses: 3, name: "Brousek", name_en: "Whetstone", icon: "🗿", type: "tool", desc: "Obtahovací kámen na ostření. Nabrousí železný nástroj bez nutnosti kovářských kleští — lehká údržba místo překování.", desc_en: "A hand whetstone for sharpening. Hones an iron tool without needing blacksmith tongs — light maintenance instead of reforging." },
    "log": { name: "Kulatina", name_en: "Log", icon: "🪵", type: "mat", desc: "Kmen pokáceného stromu. Základ tesařství.", desc_en: "Felled tree trunk. The foundation of carpentry.", seasonCat: "drevo_kura" },
    "oak_log_raw": { name: "Dubová kulatina (syrová)", name_en: "Oak Log (Green)", icon: "🪵", type: "mat", desc: "Čerstvě pokácený dub. Tvrdé, těžké dřevo — nepoužitelné rovnou, musí nejdřív schnout a zrát.", desc_en: "A freshly felled oak. Hard, heavy timber — unusable as it is, it must first dry and season." },
    "oak_log_seasoned": { name: "Dubová kulatina (vyzrálá)", name_en: "Oak Log (Seasoned)", icon: "🪵", type: "mat", desc: "Vyschlé, vyzrálé dubové dřevo. Na neustále mokrých částech vodního díla — hřídel, kolo, stavidla — tvrdne jako kámen.", desc_en: "Dried, seasoned oak. In parts that stay constantly wet — the shaft, the wheel, the sluice — it hardens like stone." },
    ...
    // kovani-rozsireni-mrd (7.8.2026): mirror metal_clasps/metal_bosses vzoru — vedlejší produkt Kovářiny
    "hrebiky": { name: "Hřebíky", name_en: "Nails", icon: "📌", type: "mat", desc: "Kované hřebíky. Drží prkna pohromadě tam, kde provaz a čep nestačí.", desc_en: "Forged nails. Hold planks together where rope and peg fall short." },
    "brass_rivet": { name: "Mosazný cvoček", name_en: "Brass Rivet", icon: "🔸", type: "mat", desc: "Malý cvoček z mosazi, ne z obyčejného železa. Kovář ho umí ukovat, ale musíš si říct — na rozdíl od hřebíků ho nedrží skladem běžně.", desc_en: "A small rivet of brass, not common iron. The blacksmith can forge one, but you must ask — unlike nails, he does not keep them in stock as a matter of course." },
    ...
    "anathema_ink": { name: "Kletebný inkoust", name_en: "Anathema Ink", icon: "🖋️", type: "mat", desc: "Duběnkový inkoust smíchaný s modlitbou. Napsaná kletba proti zlodějům drží, dokud drží stránka.", desc_en: "Gall ink mixed with a prayer. A written curse against thieves holds as long as the page does." },
    "chain_lock": { name: "Řetěz a zámek", name_en: "Chain and Lock", icon: "⛓️", type: "mat", desc: "Železný řetěz s visacím zámkem — kniha se čte na pultu, ne odnáší z místnosti.", desc_en: "An iron chain with a padlock — the book is read at the lectern, not carried from the room." },
    "libraria_secreta_kit": { name: "Trezorové okování", name_en: "Vault Fittings", icon: "🗝️", type: "mat", desc: "Pancéřované panty, dvojí zámek — jeden klíč opat, druhý celerář. Kovářská a tesařská práce dohromady, objednávka, ne řemeslo klášterní dílny.", desc_en: "Armoured hinges, a double lock — one key for the abbot, one for the cellarer. Smith's and carpenter's work together, a commission, not the monastery workshop's own craft." },
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    "quail_egg": { name: "Křepelčí vejce", name_en: "Quail Egg", icon: "🥚", type: "food", desc: "Drobounké skvrnité vajíčko z hnízda v trávě. Vzácný nález.", desc_en: "A tiny speckled egg from a nest in the grass. A rare find.", seasonCat: "divoka_vejce" },
    "duck_egg": { name: "Kachní vejce", name_en: "Duck Egg", icon: "🥚", type: "food", desc: "Vejce z divoké kachny, nalezené u vody. Větší a tučnější než slepičí.", desc_en: "A wild duck's egg, found by the water. Larger and richer than a hen's.", seasonCat: "divoka_vejce" },
    ...
    "pollen": { name: "Pyl", name_en: "Pollen", icon: "🌼", type: "mat", desc: "Včelí pyl z plástve. Sám o sobě k ničemu — teprve zkvašený s medem dá Včelí chléb.", desc_en: "Bee pollen from the comb. Useless on its own — only once fermented with honey does it become Bee Bread." },
    "bee_bread": { name: "Včelí chléb", name_en: "Bee Bread", icon: "🟫", type: "mat", desc: "Tmavá, kysele vonící hmota z plástve — pyl zkvašený s medem. Staří léčitelé (Hippokrates, Plinius) ji chválili jako posilující. Mniši věří totéž, i když nevědí proč.", desc_en: "A dark, sour-smelling mass from the comb — pollen fermented with honey. Ancient healers (Hippocrates, Pliny) praised it as fortifying. The monks believe the same, though they know not why." },
    "linden_blossom": { name: "Lipový květ", name_en: "Linden Blossom", icon: "🌸", type: "mat", desc: "Sušený lipový květ. Do čaje i léčiv.", desc_en: "Dried linden blossom. For tea and remedies.", seasonCat: "lipovy_kvet" },
    ...
    "grass": { name: "Tráva", name_en: "Grass", icon: "🌿", type: "mat", desc: "Posečená čerstvá tráva. Suší se na seno.", desc_en: "Cut fresh grass. Dried to make hay.", seasonCat: "vlakno" },
    ...
    // PISCINA (Rybník) — v8.x
    // ═══════════════════════════════════════════════════════════════════════════
    "fry": { name: "Plůdek (potěr)", name_en: "Fish Fry", icon: "🫧", type: "mat", desc: "Malý rybí potěr. Vyrůstá v rybníce.", desc_en: "Tiny fish fry. Grows in the pond.", seasonCat: "mokradni_fauna" },
    ...
    "carp": { name: "Kapr tržní", name_en: "Market Carp", icon: "🐠", type: "food_raw", desc: "Dospělý kapr. Prodej nebo vaření.", desc_en: "Adult carp. For sale or cooking.", seasonCat: "rybolov" },
    "stika": { name: "Štika", name_en: "Pike", icon: "🐊", type: "food_raw", desc: "Dravá ryba. Nasazuje se do rybníka jako přirozená kontrola hejna.", desc_en: "A predatory fish. Stocked in the pond as a natural check on the shoal.", seasonCat: "rybolov" },
    ...
    "pstruh": { name: "Pstruh", name_en: "Trout", icon: "🐡", type: "food_raw", desc: "Ryba čisté vody. Vyžaduje dobrou jakost rybníka.", desc_en: "A fish of clean water. Requires good pond quality.", seasonCat: "rybolov" },
    "uhor": { name: "Úhoř", name_en: "Eel", icon: "🐍", type: "food_raw", desc: "Tučná ryba, cenná na trhu i na stole.", desc_en: "A fatty fish, valued at market and at table.", seasonCat: "rybolov" },
    ...
    "worms": { name: "Červi", name_en: "Worms", icon: "🪱", type: "mat", desc: "Žížaly ze zahrady. Krmivo pro kapry.", desc_en: "Earthworms from the garden. Feed for carp.", seasonCat: "zizaly" },
    ...
    "circulus_lullianus": { name: "Circulus Lullianus", name_en: "Circulus Lullianus", icon: "🎡", type: "tool", desc: "Vrstvená kola z pergamenu na mosazném cvočku, popsaná vlastnostmi živlů. Podle Llullovy Ars Magna umožňují systematicky procházet kombinace, místo aby se hádalo naslepo.", desc_en: "Layered parchment wheels on a brass rivet, inscribed with the properties of the elements. Following Llull's Ars Magna, they allow combinations to be worked through systematically instead of guessed blindly." },
    ...
    // pekarna-audit v3 (30.8.2026) — nové prodejné pečivo, mirror bread_fine
    // pattern. Historicky ukotveno: perník (kořeněný medový chléb, doloženo
    // od 13. stol. ve střední Evropě, klášterní pekárny ho běžně pekly kvůli
    // "léčivým" koření), preclík (raně středověký, mnišský tvar — zkřížené
    // paže k modlitbě), postní chléb (bez tuku/medu, pro postní dny).
    "pernik": { name: "Perník", name_en: "Honey Spice Bread", icon: "🍪", type: "food", desc: "Hustý medový chléb okořeněný skořicí. Koření z něj dělá skoro lék, ne jen jídlo.", desc_en: "Dense honey bread spiced with cinnamon. The spice makes it nearly medicine, not just food." },
    "preclik": { name: "Preclík", name_en: "Pretzel", icon: "🥨", type: "food", desc: "Tvar zkřížených paží k modlitbě — mnišský vynález, prostý chléb ze slané těsta.", desc_en: "Shaped like arms crossed in prayer — a monastic invention, plain bread from salted dough." },
    "postni_chleb": { name: "Postní chléb", name_en: "Lenten Bread", icon: "🫓", type: "food", desc: "Bez tuku a medu, jen mouka a voda — na dny, kdy se drží půst.", desc_en: "No fat, no honey — just flour and water, for the days when the fast is kept." },
    ...
    "slany_sled": { name: "Slaný sleď", name_en: "Salted Herring", icon: "🐟", type: "mat", desc: "Sud nasolených sleďů dovezených od Pobaltí. Základ postní stravy, kdy jiné maso leží.", desc_en: "A barrel of salted herring imported from the Baltic. A staple of fast-day meals, when other meat is forbidden." },
    ...
    "galium": { name: "Svízel syřišťový", name_en: "Lady's Bedstraw", icon: "🌼", type: "mat", desc: "Žlutě kvetoucí bylina. Výluh sráží mléko — chudší alternativa ke slezu.", desc_en: "A yellow-flowering herb. Its extract curdles milk — a poor man's rennet.", seasonCat: "aromaticke_byliny" },
    ...
    "tvaroh": { name: "Tvaroh", name_en: "Curd", icon: "🥛", type: "mat", desc: "Mléko srazené časem a teplem, bez syřidla. Základ pro syrečky i taštičky.", desc_en: "Milk curdled by time and warmth, no rennet needed. The base for curd cheese and dumplings alike." },
    ...
    "acorn": { name: "Žalud", name_en: "Acorn", icon: "🌰", type: "mat", desc: "Dubový žalud. Prasata po nich rostou jako z vody.", desc_en: "An oak acorn. Pigs fatten on them remarkably.", seasonCat: "podzimni_orechy" },
    ...
    "tele": { name: "Tele", name_en: "Calf", icon: "🐮", type: "animal", desc: "Mladé tele. Doroste v dojnici za pár měsíců — chce chlév (kravín).", desc_en: "A young calf. Grows into a milk cow within a few months — needs a byre." },
    "byk": { name: "Býk", name_en: "Bull", icon: "🐂", type: "animal", desc: "Plemenný býk. Trvalé pořízení — na rozdíl od výpůjčky z Fora Pecuaria už neplatíš znovu.", desc_en: "A breeding bull. A permanent acquisition — unlike a loan from the Forum Pecuarium, you never pay again." },
    // polnosti-iii-vozovy-park-mrd.md v0.1 (18.9.2026): vykastrovaný býček —
    // tažné zvíře pro těžký Vozový park (Tier 2), historicky správná volba
    // pro pole a krátké cesty (oproti koni pro Tier 3, kupecký vůz). Vzniká
    // jen dorůstáním telete v chlévě, když už je v kravíně plemenný býk
    // (viz FarmyardSystem checkFarmyardTick) — nedá se koupit na Trhu.
    "vul": { name: "Vůl", name_en: "Ox", icon: "🐂", type: "animal", desc: "Vykastrovaný býk, tažné zvíře. Pomalejší než kůň, ale silnější v tahu a levnější na krmení — odjakživa první volba pro těžký žebřiňák a orbu.", desc_en: "A castrated bull, a draught animal. Slower than a horse but stronger in the pull and cheaper to feed — the traditional first choice for a heavy hay wagon and ploughing." },
    ...
    "stribrny_prut": { name: "Stříbrný prut", name_en: "Silver Ingot", icon: "🥈", type: "mat", desc: "Odlitý prut ryzího stříbra, vážený a značený zlatníkem. Cenná komodita, ne oběživo.", desc_en: "A cast bar of pure silver, weighed and marked by the goldsmith. A valuable commodity, not coin." },
    "zlaty_prut": { name: "Zlatý prut", name_en: "Gold Ingot", icon: "🟨", type: "mat", desc: "Odlitý prut ryzího zlata, vážený a značený zlatníkem. Menší poklad na dlani.", desc_en: "A cast bar of pure gold, weighed and marked by the goldsmith. A small treasure in the palm of one's hand." },
    "truhla_i": { name: "Malá truhla", name_en: "Small Chest", icon: "🗝️", type: "tool", desc: "Prostá dřevěná truhlička s železným kováním. Uschová pár cenných nálezů a kuriozit — otevře vlastní přihrádku v Personě.", desc_en: "A plain wooden chest with iron fittings. Holds a few precious finds and curiosities — opens its own compartment under Persona." },
    "truhla_ii": { name: "Velká truhla", name_en: "Large Chest", icon: "🗝️", type: "tool", desc: "Zdobená kovaná truhla se zámkem — dílo zlatníka nebo dovoz z trhu. Mnohem víc místa na poklady a kuriozity.", desc_en: "An ornate ironbound chest with a lock — a goldsmith's work or a market import. Much more room for treasures and curiosities." },
    "shelf_pluteus_inferior": { name: "Spodní zpevněný pult", name_en: "Lower Heavy Pluteus", icon: "🏛️", type: "tool", desc: "Masivní dubová deska s železnými tyčemi pro připoutání foliantů. Truhlářská práce, ne skriptorijní.", desc_en: "A massive oak board with iron bars for chaining folios. Cabinetmaker's work, not the scriptorium's own craft." },
    "shelf_pluteus_medius": { name: "Střední police", name_en: "Middle Stacks", icon: "📚", type: "tool", desc: "Řada polic ve výšce očí pro čtvrtkové kodexy a kroniky.", desc_en: "A row of shelves at eye height for quarto codices and chronicles." },
    "shelf_pluteus_superior": { name: "Horní police", name_en: "Upper Light Shelves", icon: "📖", type: "tool", desc: "Vyvýšené police pro osmerky a řeholní traktáty.", desc_en: "Raised shelves for octavos and monastic tracts." },
    "shelf_scrinium_parvum": { name: "Kapesní zásuvka", name_en: "Pocket Drawer", icon: "🗄️", type: "tool", desc: "Uzamykatelná zásuvka s plstěným polstrováním pro kapesní vademeca.", desc_en: "A lockable drawer with felt padding for pocket vademeca." },
    ...
    "sada_podkov": { name: "Sada podkov", name_en: "Horseshoe Set", icon: "🧲", type: "mat", desc: "Čtyři kované podkovy s hřeby od vesnického kováře. Bez nich kůň rychle ztrácí tažnou sílu na kamenité cestě k lomu.", desc_en: "Four forged horseshoes with nails from the village blacksmith. Without them a horse quickly loses its draught strength on the stony quarry road." },
    // kovarna-dilna-mrd.md v0.5 (30.8.2026) — 4 nové položky pro
    // podkovářský tier žebříček Kovárny.
    "worn_sada_podkov": { name: "Opotřebené podkovy", name_en: "Worn Horseshoe Set", icon: "🧲", type: "mat", desc: "Sada podkov sundaná z kopyt, tenká a popraskaná. Opravit v Kovárně.", desc_en: "A horseshoe set pulled off the hooves, thin and cracked. Repair at the Kovárna." },
    "sada_podkov_premium": { name: "Prémiové podkovy", name_en: "Premium Horseshoe Set", icon: "🧲", type: "mat", desc: "Pečlivě kovaná sada z čistšího železa, doladěná rašplí do přesnýho tvaru. Vydrží na kopytu podstatně déle.", desc_en: "A carefully forged set of purer iron, finished with a rasp to a precise fit. Lasts far longer on the hoof." },
    "podkovarske_kladivo": { name: "Podkovářské kladivo", name_en: "Farrier's Hammer", icon: "🔨", type: "tool", maxStack: 1, desc: "Lehčí a přesnější kladivo než kovářská palice — na nasazování hřebů do kopyta bez poranění zvířete.", desc_en: "A lighter, more precise hammer than a smith's mallet — for driving nails into the hoof without harming the animal." },
    "raspa_kopytni": { name: "Rašple na kopyta", name_en: "Hoof Rasp", icon: "🔧", type: "tool", maxStack: 1, desc: "Jemný pilník na doladění tvaru kopyta a podkovy. Bez něj prémiová práce nejde.", desc_en: "A fine file for finishing the shape of hoof and shoe. Premium work is impossible without it." },
    ...
    "pecena_slanecka": { name: "Pečený slaneček", name_en: "Baked Salted Herring", icon: "🐟", type: "food", hunger: 8, desc: "Sud nasolených sleďů propečený nad ohněm. Základ postní stravy.", desc_en: "Barrel-salted herring baked over the fire. A staple of fast-day meals." },
    "repny_prejt": { name: "Řepný prejt", name_en: "Turnip Pudding", icon: "🥘", type: "food", hunger: 7, desc: "Strouhaná řepa zapečená s kroupami a sádlem. Sytá zimní strava chudých.", desc_en: "Grated turnip baked with groats and lard. Hearty winter fare for the poor." },
    "raci_s_koprem": { name: "Raci s koprem", name_en: "Dill Crayfish", icon: "🦞", type: "food", hunger: 9, desc: "Raci vaření v pivu s koprem a solí, vhozeni do vroucí tekutiny živí.", desc_en: "Crayfish boiled alive in beer with dill and salt." },
    "tvarohove_tasticky": { name: "Tvarohové taštičky", name_en: "Curd Dumplings", icon: "🥟", type: "food", hunger: 10, desc: "Taštičky z mouky plněné tvarohem, vařené v hrnci.", desc_en: "Flour dumplings filled with curd, boiled in the pot." },
    ...
    "viticis_baco": { name: "Řízek Baga", name_en: "Baco Noir Cutting", icon: "🌿", type: "mat", desc: "Řízek Baco Noir (Bago). Odolný hybrid, divoce rostoucí. Vzácný nález při sběru.", desc_en: "Baco Noir (Bago) cutting. A resilient hybrid, found growing wild. A rare find.", seasonCat: "hmyz_teplo" },
...
Object.assign(ItemsDB, {
    // Suroviny — pryskyřice
    "resin_spruce": { name: "Smrková pryskyřice", name_en: "Spruce Resin", icon: "🌲", type: "mat", desc: "Obyčejná smrková smůla nasbíraná v lese. Základ nejjednodušší kadidelné směsi.", desc_en: "Common spruce pitch from the forest. The basis of the simplest incense blend.", seasonCat: "pryskyrice_smula" },
    "resin_pine": { name: "Borová pryskyřice", name_en: "Pine Resin", icon: "🌲", type: "mat", desc: "Lepkavá pryskyřice borového stromu. Dioscorides stromy Pitys a Peuce chválil pro jejich stahující vlastnosti.", desc_en: "Sticky pine resin. Dioscorides praised the Pitys and Peuce trees for their astringent properties.", seasonCat: "pryskyrice_smula" },
