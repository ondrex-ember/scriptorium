...
const StudovnaSystem = {
    ...
    isBuilt: function() {
        return !!(GameState.storage && GameState.storage.knihovna_grade_i && GameState.storage.knihovna_grade_i.built);
    },
    ...
    render: function() {
        ...
        if (!this.isBuilt()) {
            el.innerHTML = `<div style="text-align:center; padding:40px 20px; opacity:0.6;
                      ...
                      <div style="font-style:italic; font-size:0.9rem;">
                        ${lang==='en' ? 'The study room has been discovered but not yet built. Build it in Cellarium → Buildings.' : 'Studovna byla objevena, ale ještě nebyla postavena. Postav ji v Cellarium → Budovy.'}
                      </div>
                    </div>`;
            return;
        }
