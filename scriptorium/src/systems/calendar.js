...
const CalendarSystem = {
    ...
    checkCalendarEvents: function() {
        ...
        const EVENTS = [
            // A2 — Filipojakub
            { month: 4, day: 30, id: 'cal_walpurgis',
              cs: '🔥 Filipojakubská noc. Ohně hoří na kopcích. Athanor v Officině dnes září jasněji.',
              en: '🔥 Walpurgis Night. Fires burn on the hilltops. The Athanor burns brighter tonight.',
              effect: () => { if (!GameState.flags) GameState.flags = {}; GameState.flags.walpurgisBonus = true; } },
            // A4 — Máj
            { month: 5, day: 1, id: 'cal_may_day',
              cs: '🌿 Svátek máje. Zahrada se probouzí — dnešní sklizeň je dvojnásobná.',
              en: '🌿 May Day. The garden awakens — today\'s harvest is doubled.',
              effect: () => { if (!GameState.flags) GameState.flags = {}; GameState.flags.mayDayBonus = true; } },
            // A5 — Slunovrat
            { month: 6, day: 24, id: 'cal_midsummer',
              cs: '🌞 Noc svatého Jana. Byliny trhané o půlnoci mají zvláštní moc.',
              en: '🌞 Midsummer Night. Herbs gathered at midnight hold special power.',
              effect: () => { if (!GameState.flags) GameState.flags = {}; GameState.flags.midsummerBonus = true; } },
            // A6 — Dušičky
            { month: 11, day: 2, id: 'cal_all_souls',
              cs: '🕯️ Dušičky. Svíčky hoří na hrobech. Titivillus je tuto noc živější.',
              en: '🕯️ All Souls\' Day. Candles burn on graves. Titivillus stirs more than usual tonight.',
              effect: null },
            // A7 — Advent
            { month: 12, day: 1, id: 'cal_advent',
              cs: '✝️ Advent začal. Čtyři neděle čekání. Poptávka po žaltářích roste.',
              en: '✝️ Advent has begun. Four weeks of waiting. Demand for psalters is rising.',
              effect: null },
            // A8 — Štědrý
            { month: 12, day: 24, id: 'cal_christmas',
              cs: '⭐ Štědrý večer. Cellarium zavřeno. Komunita odpočívá. Pax et bonum.',
              en: '⭐ Christmas Eve. The Cellarium is closed. The community rests. Pax et bonum.',
              effect: null },
            // A9 — Silvestr
            { month: 12, day: 31, id: 'cal_new_year',
              cs: '🎉 Poslední den roku. Kronikář zapíše poslední řádku. Čistá stránka čeká.',
              en: '🎉 Last day of the year. The chronicler writes the final line. A clean page awaits.',
              effect: null },
            // A9b — Nový rok
            { month: 1, day: 1, id: 'cal_new_year_b',
              cs: '🎉 Nový rok. Calendae Ianuariae. Calendarium obnovit!',
              en: '🎉 New Year. Calendae Ianuariae. Remember to renew the Calendarium!',
              effect: null },
        ];

        // Pohyblivé
        const ashDate = new Date(easterDate); ashDate.setDate(easterDate.getDate() - 46);
        if (ashDate.getMonth()+1 === month && ashDate.getDate() === day) {
            EVENTS.push({ id: 'cal_ash_wednesday',
                cs: '✝️ Popeleční středa. Čtyřicet dní půstu začíná. Vigor produkce klesá.',
                en: '✝️ Ash Wednesday. Forty days of fasting begin. Vigor production decreases.',
                effect: null });
        }
        if (easter.month === month && easter.day === day) {
            EVENTS.push({ id: 'cal_easter',
                cs: '✝️ Velikonoce. Christus resurrexit! Vigor +30. Cellarium otevřeno celý den.',
                en: '✝️ Easter. Christus resurrexit! Vigor +30. The Cellarium is open all day.',
                effect: () => { if (typeof VigorSystem !== 'undefined') VigorSystem.restore('easter'); } });
        }

        EVENTS.forEach(ev => {
            if (ev.month && ev.month !== month) return;
            if (ev.day && ev.day !== day) return;
            if (GameState.flags[`calEvent_${ev.id}_${year}`]) return;
            GameState.flags[`calEvent_${ev.id}_${year}`] = true;
            const msg = lang === 'en' ? ev.en : ev.cs;
            if (typeof NotificationSystem !== 'undefined') NotificationSystem.panel(msg, 'system');
