...
const FireplaceSystem = {
    ...
    PIPE_PARANOIA_FLASHES: [
        ...
        { cs: "Kočka na tebe mluví lidskou řečí. Řekla: „Mám hlad.“", en: "The cat speaks to you in human words. She said: \"I'm hungry.\"" },
        { cs: "Zvířata na dvoře se chovají podezřele, koukají na tebe podezíravě.", en: "The animals in the yard are acting suspicious, eyeing you warily." },
        { cs: "Zahrádka roste před očima.", en: "The garden is growing right before your eyes." },
        { cs: "Kočka na tebe mluví lidskou řečí. Řekla: „Co čumíš, koukej mi dát něco k jídlu.“", en: "The cat speaks to you in human words. She said: \"What are you staring at, go get me something to eat.\"" },
        { cs: "Kočka na tebe mluví lidskou řečí. Řekla: „Proč mě nehladíš?“", en: "The cat speaks to you in human words. She said: \"Why aren't you petting me?\"" },
        { cs: "Při procházce sadem jsi měl tíživý pocit, že tě někdo sleduje.", en: "Walking through the orchard, you felt a heavy sense that someone was watching you." },
        { cs: "No teda, zakouřit si toto po pár pivech tak nevím.", en: "Well, smoking this after a few beers... you're not so sure about that." },
        { cs: "Zapomněl jsi, cos právě dělal. Možná jsi nedělal nic. Možná to bylo geniální.", en: "You forgot what you were just doing. Maybe you weren't doing anything. Maybe it was brilliant." },
        { cs: "Titivillus sedí vedle tebe a mlčky sleduje, jak dýcháš.", en: "Titivillus is sitting beside you, silently watching you breathe." },
        { cs: "Ruce se ti zdají obrovské. Kdy jsi je naposledy pořádně viděl?", en: "Your hands seem enormous. When did you last really look at them?" },
        { cs: "Latinské slovo, které píšeš celý život, najednou vypadá jako vymyšlené.", en: "The Latin word you've written your whole life suddenly looks made up." },
        { cs: "Zvon odbil poledne. Nebo večerku. Nebo obojí najednou.", en: "The bell rang for noon. Or compline. Or both at once." },
        { cs: "Hlad přišel znenadání a s mocí trojnásobnou. Kde je ten chleba?", en: "Hunger struck suddenly, threefold. Where's that bread?" },
        { cs: "Kámen ve zdi má tvář. Zírá na tebe už tři sta let.", en: "The stone in the wall has a face. It's been staring at you for three hundred years." },
        { cs: "Napadlo tě, jestli mají ryby v rybníce svého opata.", en: "You wonder if the fish in the pond have their own abbot." },
        { cs: "Bosá noha na dlažbě cítí každý kamínek zvlášť, jako by ti něco šeptal.", en: "Your bare foot on the flagstones feels every pebble separately, as if each one were whispering." },
        { cs: "Bratr Bernard řekl něco před hodinou. Pořád se tomu směješ.", en: "Brother Bernard said something an hour ago. You're still laughing about it." },
        { cs: "Kdyby ryba měla nohy, byla by žába. Zajímalo by tě, kdo o tom rozhodl.", en: "If a fish had legs, it would be a frog. You wonder who decided that." },
        { cs: "Zdálo se ti, že Giacomo stojí ve dveřích a jen se usmívá. Nic neprodává. Jen se usmívá.", en: "Giacomo seemed to be standing in the doorway, just smiling. Selling nothing. Just smiling." },
        { cs: "Svíčka hoří tak krásně. Jak dlouho už se na ni díváš?", en: "The candle burns so beautifully. How long have you been staring at it?" },
    ],
    ...
    smokePipe: function() {
        ...
        if (_abbotHere && Math.random() < 0.4 && typeof TemplumSystem !== 'undefined' && !TemplumSystem.isSuspended()) {
            ...
            if (hitFloor || hitLimit) {
                ...
                this._sendAbbotSmokeWarning(warnCount, true, lang);
            } else {
                this._sendAbbotSmokeWarning(warnCount, false, lang);
            }
        }
    ...
    // dymka-templum-mrd (9.8.2026), revize "žádný holub" — opat je FYZICKY
    // přítomen, takže žádná pigeon-fikce: dopis jde přímo do Porty,
    // podepsaný živým jménem opata (AbbotSystem.CURRENT.name — přežije
    // budoucí výměnu opata, viz abbot-lineage-mrd). Pokud hráč ještě nemá
    // Portu odemčenou (GameState.flags.porta_active), spadne to zpátky na
    // obyčejnou notifikaci — jinak by dopis zmizel v datech beze stopy,
    // Porta tab před odemčením v navigaci vůbec neexistuje.
    _sendAbbotSmokeWarning: function(warnCount, isSuspension, lang) {
        const abbotName = (typeof AbbotSystem !== 'undefined' && AbbotSystem.CURRENT && AbbotSystem.CURRENT.name)
            || (lang === 'en' ? 'The Abbot' : 'Opat');

        const TEXTS = {
            1: {
                cs: `Bratře, přistihl jsem tě s dýmkou v hodinách, kdy má vonět jen kadidlo. Netrestám — jen připomínám: dým ať zůstane u ohniště, ne u tvé práce. — ${abbotName}`,
                en: `Brother, I caught you with your pipe at an hour that should smell of incense alone. I do not punish — only remind: let the smoke stay by the hearth, not by your work. — ${abbotName}`
            },
            2: {
                cs: `Podruhé, bratře. Dým z tvé dýmky mě už zdaleka nemine — a moje trpělivost houstne stejně pomalu. Příště se neomezím na slovo. — ${abbotName}`,
                en: `A second time, brother. The smoke of your pipe no longer escapes me from afar — and my patience thickens just as slowly as it does. Next time, words will not be enough. — ${abbotName}`
            },
            suspension: {
                cs: `Řekl jsem to už dvakrát. Dnes jsem to viděl na vlastní oči potřetí — kostel je ti do odvolání uzavřen. Vrať se, až tvé pokání bude vidět, ne jen slyšet. — ${abbotName}`,
                en: `I said it twice already. Today I saw it with my own eyes a third time — the church is closed to you until further notice. Return when your penance can be seen, not merely heard. — ${abbotName}`
            }
        };
        const key = isSuspension ? 'suspension' : (warnCount >= 2 ? 2 : 1);
        const body = TEXTS[key];
        const titleCs = isSuspension ? 'Kostel uzavřen' : `Varování ${warnCount}/${TemplumSystem.SUSPENSION_WARNING_LIMIT}`;
        const titleEn = isSuspension ? 'Church Closed' : `Warning ${warnCount}/${TemplumSystem.SUSPENSION_WARNING_LIMIT}`;

        const portaReady = GameState.flags && GameState.flags.porta_active && typeof PortaSystem !== 'undefined';
        if (portaReady) {
            PortaSystem._ensureState();
            const letterId = 'abbot_smoke_' + (isSuspension ? 'susp' : warnCount) + '_' + Date.now();
            GameState.letters.dynamic.push({
                id: letterId,
                sender_cs: 'Opat ' + abbotName, sender_en: 'Abbot ' + abbotName,
                seal: 'abbot',
                title_cs: titleCs, title_en: titleEn,
                text_cs: body.cs, text_en: body.en,
                choices: [{
            ...
            NotificationSystem.panel(lang === 'en'
                ? '✉️ A note has arrived from the Abbot.'
                : '✉️ Dorazil vzkaz od opata.', 'porta');
        } else {
            NotificationSystem.panel(lang === 'en' ? '✉️ ' + body.en : '✉️ ' + body.cs, 'warning');
        }
    },
