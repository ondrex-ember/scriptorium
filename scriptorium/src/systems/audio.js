...
class AudioSystem {
    ...
    switchMusicTier(tier) {
        if (tier === 0) {
            // Žádná hudba — fade out
            this.musicGain.gain.setTargetAtTime(0, this.audioContext.currentTime, 1.0);
            if (this.music) { this.music.stop(); this.music = null; }
            this.musicTier = 0;
            console.log('🎵 Hudba: vypnuta (žádný tier odemčen)');
            return;
        }

        if (this.musicTier === tier && this.music && this.music._started) return; // Už hraje správný tier

        // Stopnout aktuální engine
        if (this.music) {
        ...
        setTimeout(() => {
            if (tier === 1) {
                ...
                console.log('🎵 Hudba: Tier 2 — Abyssal Keep');
            } else if (tier === 3) {
                this.music = new ArsNovaGenerative(this.audioContext, this.musicGain);
                console.log('🎵 Hudba: Tier 3 — Ars Nova');
            }
            this.musicTier = tier;
