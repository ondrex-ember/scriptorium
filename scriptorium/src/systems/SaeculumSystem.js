...
const SaeculumSystem = {
  ...
  renderDormitoriumDetail: function(b) {
    ...
    specKeys.forEach(tabId => {
      ...
      // Infirmarium mnišský role — tech-gated; Studovna — gated na postavenou Knihovnu St. I;
      // Kantor — gated na Schola Cantorum.
  ...
  renderConversiDetail: function(k) {
    ...
    const rec = (k.rosterId && typeof ConversiRosterDB !== 'undefined') ? ConversiRosterDB[k.rosterId] : null;
    const icon = (rec && rec.icon) ? rec.icon : '✝️';
    let origin = rec ? (lang === 'en' ? rec.origin_en : rec.origin_cs) : '';
    // Vlákno C — Famulus/Oblát nemají pojmenovaný roster (rec === null),
    // příběh se odemyká postupně podle dní ve službě (AnonymousLoreDB).
    if (!rec && (k.type === 'famulus' || k.type === 'oblat') && typeof AnonymousLoreDB !== 'undefined') {
      const pool = AnonymousLoreDB[k.type] || [];
      const daysServed = k.hiredAt ? Math.floor((Date.now() - k.hiredAt) / (24*60*60*1000)) : 0;
      const unlocked = pool.filter(fr => daysServed >= fr.days);
      if (unlocked.length) {
        const latest = unlocked[unlocked.length - 1];
        origin = lang === 'en' ? latest.en : latest.cs;
      }
    }
    const mood = (typeof k.mood === 'number') ? k.mood : 60;
    const loyalty = (typeof k.loyalty === 'number') ? k.loyalty : 30;
  ...
  // clientela-sell-cap-variabilni (15.8.2026): strop podle ceny (proxy pro
  // vzácnost/realitu) — levné suroviny se hromadí (vyšší strop), drahé/
  // vzácné zboží má přirozeně omezenější odbyt. Nahrazuje flat číslo
  // (bylo 30, dřív 15) — malý edit, ne per-item konfigurace v ContactsDB.
  _contactSellCap: function(basePrice) {
    if (basePrice <= 5) return 40;
    if (basePrice <= 15) return 25;
    return 12;
  },
  ...
  sellToContact: function(contactId, itemId, qty) {
    ...
    const result = CellariumSystem._calcSaturatedSale(relPrice, qty, soldBefore, this._contactSellCap(basePrice));
  ...
  buyFromContact: function(contactId, itemId, qty) {
    ...
    const unitPrice = Math.max(1, Math.round(offer.price * this.contactBuyDiscountMult(contactId)));
    ...
    CellariumSystem.recordTransaction('buy', itemId, qty, unitPrice, contactId);
    this.addContactRelation(contactId, 1);
    Game.save();
    const lang = (GameState.settings && GameState.settings.language) || 'cs';
    const itemName = (typeof iName === 'function') ? iName(itemId) : itemId;
  ...
  renderContactPanel: function(id) {
    ...
    if (offerItems && Object.keys(offerItems).length) {
      Object.keys(offerItems).forEach(itemId => {
        ...
        const priceLabel = discMult < 1
          ? `<span style="text-decoration:line-through; opacity:0.5;">${o.price}</span> ${unitPrice} g/${lang==='en'?'pc':'ks'}`
          : `${unitPrice} g/${lang==='en'?'pc':'ks'}`;
        h += `<div style="display:flex; align-items:center; gap:6px; font-size:0.78rem; margin-bottom:6px;">
                <span style="flex:1;">${itemName} <span style="opacity:0.6;">(${priceLabel} · ${lang==='en'?'stock':'skladem'} ${left})</span></span>
                <button class="craft-btn" style="padding:2px 8px; font-size:0.7rem;" ${left>0 && grose>=unitPrice ? '' : 'disabled'} onclick="SaeculumSystem.buyFromContact('${id}','${itemId}',1)">×1</button>
                <button class="craft-btn" style="padding:2px 8px; font-size:0.7rem;" ${left>=5 && grose>=unitPrice*5 ? '' : 'disabled'} onclick="SaeculumSystem.buyFromContact('${id}','${itemId}',5)">×5</button>
              </div>`;
      });
    } else {
    ...
    if (sbItems && Object.keys(sbItems).length) {
      ...
      Object.keys(sbItems).forEach(itemId => {
        ...
        const left = Math.max(0, this._contactSellCap(basePrice) - soldToday);
