...
const VigorSystem = {
    ...
    SATIETY_DRAIN_PER_HOUR: 0.5,   // 100→0 za ~8.3 dne

    // Fatigue recovery per hodina (záporné = klesá)
    FATIGUE_RECOVERY_DAY:   2,     // 6–18h: -2/h
    FATIGUE_RECOVERY_NIGHT: 3,     // 18–6h: -3/h
    FATIGUE_RECOVERY_HORA:  4,     // Completorium/Vigilie: -4/h
    ...
    _applyOfflineDelta: function() {
        ...
        // Satiety drain
        const satDrain = hoursElapsed * this.SATIETY_DRAIN_PER_HOUR;
    ...
    _tick: function() {
        ...
        // Satiety drain
        const satDrain = elapsed * this.SATIETY_DRAIN_PER_HOUR;
        GameState.satiety = Math.max(0, (GameState.satiety || 80) - satDrain);

        // Fatigue recovery dle denní doby
        const hour = new Date().getHours();
        let recovery = this.FATIGUE_RECOVERY_DAY;
        if (hour < 6 || hour >= 18) recovery = this.FATIGUE_RECOVERY_NIGHT;
    ...
    // ── Legacy renderMiniDisplay (pro kompatibilitu) ──────────────────────────
