...
const VigorSystem = {
    ...
    _tick: function() {
        ...
        if (meta.nonaStart > 0 && now - meta.nonaStart >= 60000) {
            ...
            const msg = lang === 'en'
                ? `😴 Nona rest complete. Fatigue -20. Vigor: ${this.getVigor()}.`
                : `😴 Odpočinek při Nóně dokončen. Únava -20. Vigor: ${this.getVigor()}.`;
    ...
    // ── Meditace (1× za 12h, trvá 3 min real time) ──────────────────────────
    ...
    renderFullDisplay: function() {
        ...
        const nonaBtn = nonaActiveFd
            ? `<div style="margin-top:8px;font-size:0.7rem;color:var(--accent-gold);opacity:0.8;">😴 ${lang === 'en' ? `Resting... ${nonaRemainSFd}s` : `Odpočíváš... ${nonaRemainSFd}s`}<div style="height:3px;background:rgba(197,160,89,0.15);border-radius:2px;margin-top:4px;"><div style="height:3px;background:var(--accent-gold);border-radius:2px;width:${Math.round((1-nonaRemainSFd/60)*100)}%;"></div></div></div>`
            : nonaUsedTodayFd
            ? `<div style="font-size:0.7rem;opacity:0.5;margin-top:6px;">${lang === 'en' ? 'Nona rest used today.' : 'Dnes již odpočinut.'}</div>`
