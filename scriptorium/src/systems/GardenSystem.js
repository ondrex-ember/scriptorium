...
const GardenSystem = {
    ...
    waterVine: function(idx) {
        ...
        let source = null;
        if ((GameState.inventory['water'] || 0) >= waterCost) source = 'water';
        else if ((GameState.inventory['spring_water'] || 0) >= waterCost) source = 'spring_water';
        if (!source) {
            UI.notify(lang==='en' ? 'Not enough water.' : 'Nedostatek vody!', true); return;
        }
        Game.removeItem(source, waterCost);
        slot.lastWateredAt = Date.now();
        Game.save();
        this.renderVinohrad();
        if (source === 'spring_water') {
            UI.notify('💧 ' + (lang==='en' ? 'Watered with spring water.' : 'Zalito pramenitou vodou.'));
        } else {
    ...
    renderFieldTab: function() {
        ...
        const springForField = GameState.inventory['spring_water'] || 0;
        ...
        GameState.fields.forEach((field, idx) => {
            ...
            if (field.state === 'empty') {
                ...
                if (!field.watered && phaseIdx < 3) {
                    btn = `<button class="craft-btn" onclick="GardenSystem.waterField(${idx})" ${(water>=waterCost || springForField>=waterCost)?'':'disabled'}>💧 ${lang==='en'?'Water':'Zalít'} (${waterCost}💧)</button>`;
                } else if (phaseIdx >= 3) {
    ...
    waterField: function(idx) {
        ...
        field.watered = true;
        Game.save();
        if (source === 'spring_water' && typeof UI !== 'undefined') {
            UI.notify('💧 ' + (lang==='en' ? 'Watered with spring water.' : 'Zalito pramenitou vodou.'));
        }
        this.renderFieldTab();
    },
        ...
        GameState.vinea.forEach((slot, idx) => {
            ...
            if (slot.state === 'empty') {
                ...
                btn = `<button class="craft-btn" onclick="GardenSystem.pruneVine(${idx})" ${slot.pruned?'disabled':''}>✂️ ${lang==='en'?'Prune':'Prořezat'}${slot.pruned?' ✓':''}</button>
                       <button class="craft-btn" onclick="GardenSystem.waterVine(${idx})" style="font-size:0.72rem; margin-top:3px;" ${recentlyWatered?'disabled':''}>💧 ${lang==='en'?'Water':'Zalít'}</button>
