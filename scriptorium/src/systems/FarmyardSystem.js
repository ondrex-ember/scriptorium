...
const FarmyardSystem = {
    ...
    ANIMAL_CFG: {
        ...
        goatpen: {
            ...
            breedMs: 30 * 24 * 60 * 60 * 1000  // březost kozy (goat-mrd, mirror prase/kráva)
        },
        cowbyre: {
    ...
    // historicky-podklad-mrd (9.8.2026): stable doplněno — jen kvůli
    // shoeDurability decay v moodTick (nálada samotná jde přes v2/getMood,
    // viz restrukturovaný moodTick níž).
    ALL_PENS: ['rabbitry', 'goatpen', 'pigsty', 'donkeyStall', 'cowbyre', 'stable'],
    // v2 mood systém (hunger+hygiene přes getMood) — tyhle výběhy už nemají
    // pasivní decay mutující a.mood. historicky-podklad-mrd (9.8.2026):
    // stable+donkeyStall doplněny pro konzistenci (dřív mimo v2 úplně).
    MOOD_V2_PENS: ['rabbitry', 'henhouse', 'sheepfold', 'goatpen', 'cowbyre', 'pigsty', 'stable', 'donkeyStall'],
    ...
    renderDonkeyStall: function () {
        ...
        // historicky-podklad-mrd (9.8.2026): stejný hay-feed vzor jako
        // goatpen/cowbyre/pigsty/stable (renderAnimalPen) — dřív žádný.
        ...
        if (st.animals.length) {
            ...
            const donkeyMood = this.getMood('donkeyStall');
            ...
            st.animals.forEach((a, i) => {
                this._ensureAnimalFields(a);
                const mIcon = this.MOOD_ICON(donkeyMood);
                h += `<div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
                    <span style="font-size:1.4rem;">🫏</span>
                    <div style="flex:1;">
                        <span style="font-weight:bold;">${a.name || t('farmyard.donkeyDefault')}</span>
                        <span style="font-size:0.75rem; opacity:0.6; margin-left:6px;">${mIcon} ${donkeyMood}/100</span>
    ...
    _pigMature: function (a) {
        return Date.now() - a.placedAt >= (a.growTargetMs || this.ANIMAL_CFG.pigsty.growMs);
    },

    // Tele → kráva (krava-mrd, 26.7.2026), mirror _pigMature
    _calfMature: function (a) {
        return Date.now() - (a.placedAt || a.bornAt) >= this.ANIMAL_CFG.cowbyre.growMs;
    },
    ...
    placeAnimal: function (pen, itemId) {
        ...
        // Skot (cowbyre) — tři typy ve stejném chlévě, rozliš podle itemId (krava-mrd, 26.7.2026)
        if (pen === 'cowbyre') {
            a.type = useItemId;             // 'tele' | 'cow' | 'byk'
            if (useItemId === 'tele') { a.mature = false; a.placedAt = Date.now(); }
        }
        // historicky-podklad-mrd (9.8.2026): sele → dospělé prase, 180-240
        // dní (6-8 měsíců), vážené k nižšímu konci (pow 1.6 stáhne náhodu
        // k nule). Dřív tu chybělo a.placedAt úplně (bug, viz _pigMature).
        if (pen === 'pigsty') {
            a.mature = false;
            a.placedAt = Date.now();
            a.growTargetMs = 180 * 86400000 + Math.floor(Math.pow(Math.random(), 1.6) * 60 * 86400000);
        }
    ...
    slaughterCow: function (idx) {
        ...
        // krava-mrd (26.7.2026): chlév teď drží i tele/býka — cílit jen na krávu,
        // ne slepě na poslední index (nezabít omylem plemenného býka)
        if (idx === undefined || (st.animals[idx] && st.animals[idx].type && st.animals[idx].type !== 'cow')) {
            idx = -1;
            for (let i = st.animals.length - 1; i >= 0; i--) {
                if (!st.animals[i].type || st.animals[i].type === 'cow') { idx = i; break; }
            }
        }
        const a = st.animals[idx];
    ...
    renderAnimalPen: function (pen) {
        ...
        // historicky-podklad-mrd (9.8.2026): seno-krmení pro goatpen/
        // cowbyre/pigsty/stable — v teplé sezóně se zvěř pase sama zdarma
        // (sezónní gating přijde samostatně), tohle je jen samotné Feed
        // tlačítko + funkce, co dřív chyběly úplně (viz feedPen).
        if (['goatpen', 'cowbyre', 'pigsty', 'stable'].includes(pen) && st.animals.length > 0) {
        ...
        if (pen === 'goatpen' && st.animals.length) {
            ...
            h += '</div>';
            // Plemenitba — mirror prasečího/kravího bloku (goat-mrd)
            var hasDoe = st.animals.some(function (a) { return a.mature !== false; });
            h += '<div style="margin-top:6px; padding:10px; background:rgba(0,0,0,0.06); border-radius:8px;">';
            h += '<strong style="font-size:0.85rem;">🐐 ' + (t('farmyard.goatBreeding') || 'Plemenitba') + '</strong><br>';
            if (!st.breeding) {
                var canBreedG = hasDoe && _billyActive;
                var breedLabelG = !hasDoe ? (t('farmyard.needDoe') || 'Potřebuješ dospělou kozu.') : !_billyActive ? (t('farmyard.needBilly') || 'Potřebuješ kozla (Forum Pecuarium).') : (t('farmyard.goatStartBreeding') || 'Zahájit odchov');
                h += '<button class="craft-btn" onclick="FarmyardSystem.startGoatBreeding()" ' + (canBreedG ? '' : 'disabled') + ' style="margin-top:6px;font-size:0.78rem;">' + breedLabelG + '</button>';
            } else if (st.breeding.state === 'gestating') {
                var leftG = Math.max(0, Math.ceil((st.breeding.bornAt - now2) / 3600000));
                h += '<p class="text-sm" style="margin:6px 0;">🤰 ' + (t('farmyard.goatGestating') || 'Březí') + ' — ' + leftG + 'h</p>';
            }
            h += '</div>';
        }
        ...
        if (pen === 'cowbyre' && st.animals.length) {
            ...
            st.animals.forEach(function (a) {
                if (a.type !== 'tele') return;
                var pctT = Math.min(100, Math.round((Date.now() - (a.placedAt || a.bornAt)) / _selfCB.ANIMAL_CFG.cowbyre.growMs * 100));
                h += '<div style="padding:6px 10px;background:rgba(0,0,0,0.04);border-radius:6px;margin-bottom:6px;font-size:0.78rem;">🐮 ' + (t('dvur.calfGrowing') || 'Tele dorůstá') + ' — ' + pctT + '%</div>';
            });
            h += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px;">';
        ...
        if (pen === 'pigsty' && st.animals.length) {
            ...
            st.animals.forEach(function (a, i) {
                ...
                var pct = Math.min(100, Math.round((Date.now() - (a.placedAt || a.bornAt || Date.now())) / (a.growTargetMs || _selfAct.ANIMAL_CFG.pigsty.growMs) * 100));
        ...
        if (pen === 'stable' && st.animals.length) {
            var stableMood = this.getMood('stable');
            h += '<div style="display:flex;flex-direction:column;gap:6px;margin-bottom:10px;">';
    ...
    // Skot — mirror startBreeding (ovce), jen delší gestace a samec může
    // být buď vypůjčený (Forum Pecuarium) NEBO trvale vlastněný býk v chlévě
    // (krava-mrd, 26.7.2026).
    hasBullAvailable: function () {
        const st = GameState.cowbyre;
        return this.loanMaleActive('bull') || !!(st && st.animals && st.animals.some(a => a.type === 'byk'));
    },

    startCowBreeding: function () {
        const st = GameState.cowbyre;
        if (!st || !st.built) return;
        const cows = st.animals.filter(a => a.type === 'cow' && a.mature !== false);
        if (!cows.length) { UI.notify(t('farmyard.needCow') || 'Potřebuješ dojnici.', true); return; }
        if (st.breeding) { UI.notify(t('game.breedingActive'), true); return; }
        if (!this.hasBullAvailable()) { UI.notify(t('farmyard.needBull') || 'Potřebuješ býka (koupě nebo Forum Pecuarium).', true); return; }
        st.breeding = { state: 'gestating', startedAt: Date.now(), bornAt: Date.now() + this.ANIMAL_CFG.cowbyre.breedMs };
        Game.save(); FarmyardSystem.renderFarmyard();
    ...
    // Koza — mirror startPigBreeding (goat-mrd). Vrh 1-2 kůzlata (kozy běžně
    // dvojčata), výstup jde do inventáře (goat), ne rovnou do chlévě —
    // chlév může být plný, hráč umístí přes existující placeAnimal('goatpen').
    startGoatBreeding: function () {
        const st = GameState.goatpen;
        if (!st || !st.built) return;
        const does = st.animals.filter(a => a.mature !== false);
        if (!does.length) { UI.notify(t('farmyard.needDoe') || 'Potřebuješ dospělou kozu.', true); return; }
        if (st.breeding) { UI.notify(t('game.breedingActive'), true); return; }
        if (!this.loanMaleActive('billy_goat')) { UI.notify(t('farmyard.needBilly') || 'Potřebuješ kozla (Forum Pecuarium).', true); return; }
        st.breeding = { state: 'gestating', startedAt: Date.now(), bornAt: Date.now() + this.ANIMAL_CFG.goatpen.breedMs };
        Game.save(); FarmyardSystem.renderFarmyard();
        UI.notify('🐐 ' + (t('farmyard.goatBreedingStarted') || 'Plemenitba zahájena.'));
    },
    ...
    // Porta dopis v letters.js) — kterákoliv nastane dřív, vyhrává; obě
    // volají tuhle jednu funkci. Prodej NEuzavírá navždy — šance se jen
    // násobí 0.5 za každý prodej, poražení/umístění uzavírá navždy.
    // ═══════════════════════════════════════════════════════════════════════
    STRAY_COW_BASE_CHANCE: 0.01,

    strayCowChance: function () {
        if (!GameState.strayCow) GameState.strayCow = { resolved: false, soldCount: 0 };
        if (GameState.strayCow.resolved) return 0;
        return this.STRAY_COW_BASE_CHANCE * Math.pow(0.5, GameState.strayCow.soldCount || 0);
    },

    showStrayCowModal: function (source) {
        let existing = document.getElementById('stray-cow-modal');
        if (existing) existing.remove();
        const lang = (GameState.settings && GameState.settings.language) || 'cs';
        const cs = lang !== 'en';
        const modal = document.createElement('div');
        modal.id = 'stray-cow-modal';
          ...
          <div style="background:var(--bg-parchment);border:2px solid var(--accent-gold);border-radius:12px;
            ...
            <div style="text-align:center;margin-bottom:18px;">
              ...
              <div style="font-family:'Cinzel Decorative';font-size:1.1rem;color:var(--accent-gold);">
                ${cs ? 'Zatoulaná kráva' : 'A Stray Cow'}
              </div>
            </div>
            <div style="font-style:italic;font-size:0.9rem;opacity:0.85;margin-bottom:22px;
                        padding:15px;background:rgba(197,160,89,0.08);border-radius:8px;
                        border-left:3px solid var(--accent-gold);">
              ${cs ? 'Kráva bez pána, zmožená a hubená. Co s ní?' : 'A cow with no owner, weak and thin. What now?'}
            </div>
            <div style="display:flex;flex-direction:column;gap:10px;">
              <button onclick="FarmyardSystem._strayCowResolve('place')" class="craft-btn">
                🏠 ${cs ? 'Umístit (do inventáře, potřebuje chlév)' : 'Take her in (to inventory, needs a byre)'}
              </button>
              <button onclick="FarmyardSystem._strayCowResolve('slaughter')" class="craft-btn" style="background:#8b4a3a;color:#fff;">
                🔪 ${cs ? 'Utratit — je zmožená' : 'Put her down — she is too weak'}
              </button>
              <button onclick="FarmyardSystem._strayCowResolve('sell')" class="craft-btn" style="background:var(--accent-gold);color:var(--bg-parchment);">
                💰 ${cs ? 'Prodat pryč (50g)' : 'Sell her off (50g)'}
              </button>
            </div>
          </div>
        `;
    ...
    _strayCowResolve: function (choice) {
        const el = document.getElementById('stray-cow-modal');
        if (el) el.remove();
        if (!GameState.strayCow) GameState.strayCow = { resolved: false, soldCount: 0 };
        if (choice === 'place') {
            Game.addItem('cow', 1);
    ...
    // historicky-podklad-mrd (9.8.2026): sdílená feed funkce pro
    // goatpen/cowbyre/pigsty/stable — dřív neexistovala vůbec (bug, viz
    // audit). 1 seno + 1 voda per zvíře, mirror feedRabbitry/feedSheepfold.
    feedPen: function (pen) {
        const st = GameState[pen];
    ...
    checkFarmyardProduction: function () {
        ...
        // Skot — gestace → nové tele přímo do chlévě (krava-mrd, 26.7.2026),
        // mirror ovčí gestace, jen bez mezistavu "growing"/lambPool — tele
        // rovnou dorůstá v chlévě přes _calfMature/growMs.
        const cowSt = GameState.cowbyre;
        if (cowSt && cowSt.breeding && cowSt.breeding.state === 'gestating' && now >= cowSt.breeding.bornAt) {
            cowSt.breeding = null;
            if (cowSt.animals.length < this.ANIMAL_CFG.cowbyre.cap) {
                cowSt.animals.push({ type: 'tele', sex: Math.random() < 0.5 ? 'm' : 'f', mood: 80, mature: false, bornAt: now, placedAt: now, lastCleaned: 0 });
                if (typeof UI !== 'undefined') UI.notify('🐮 ' + (t('farmyard.calfBorn') || 'Narodilo se tele.'));
                if (typeof Game !== 'undefined' && Game.addKronikaEntry) Game.addKronikaEntry('event', '🐮 V kravíně se narodilo tele.', '🐮 A calf was born in the byre.', '🐮 Vitulus natus est.');
            }
            changed = true;
        }
        ...
        const goatSt = GameState.goatpen;
        if (goatSt && goatSt.breeding && goatSt.breeding.state === 'gestating' && now >= goatSt.breeding.bornAt) {
            goatSt.breeding = null;
            const kids = 1 + Math.floor(Math.random() * 2);
            GameState.inventory.goat = (GameState.inventory.goat || 0) + kids;
            if (typeof UI !== 'undefined') UI.notify('🐐 ' + (t('farmyard.kidsBorn') || 'Narodila se {n} kůzlata.').replace('{n}', kids));
            if (typeof Game !== 'undefined' && Game.addKronikaEntry) Game.addKronikaEntry('event', '🐐 V kozinci se narodila kůzlata.', '🐐 Kids were born in the goat pen.', '🐐 Haedi nati sunt.');
            changed = true;
        }
