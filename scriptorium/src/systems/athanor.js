...
const AthanorSystem = {
  ...
  tick() {
    ...
    if (GameState.athanor.brewing) {
      ...
      // NOVÉ: vaření ještě běží — překreslit, ať countdown vizuálně žije (ne jen na re-render při navigaci)
  ...
  buildStatsBar(lunar, canonical, state) {
    ...
    // Destilační vybavení — persistentní počty (ne jen varování při nule), viditelné
    // hned v status baru, ať nemusíš nejdřív kliknout na Destilaci a dostat toast.
    const hasDestilTech = GameState.researchedTechs && GameState.researchedTechs.includes('tech_destillatio');
    const lang = (GameState.settings && GameState.settings.language) || 'cs';
    let destilGear = '';
    if (hasDestilTech) {
      const alembicQty = GameState.inventory['alembic'] || 0;
      const flaskQty = GameState.inventory['glass_flask'] || 0;
      const alembicColor = alembicQty <= 0 ? '#e05a3a' : '#c9a96e';
      const flaskColor = flaskQty <= 0 ? '#e05a3a' : '#c9a96e';
      const glassmakerHint = lang === 'en' ? 'The Glassmaker sells these' : 'Sklář je prodá';
      destilGear = `<span style="opacity:0.3;">·</span>
        <span style="color:${alembicColor};" title="${lang==='en'?'Alembic (tool, reusable)':'Alembik (nástroj, nespotřebuje se)'} — ${glassmakerHint}">🧪 ${alembicQty}</span>
        <span style="color:${flaskColor};" title="${lang==='en'?'Glass flask (consumed per distillation)':'Skleněná baňka (spotřebuje se za běh)'} — ${glassmakerHint}">🫙 ${flaskQty}</span>`;
    }
  ...
  buildAlembicSvg(state) {
    ...
    const isMacerating = procId === 'maceratio';
    const now = Date.now();
    let pct = 0, si = 0;
    if (isBrewing) {
    ...
    const lit = isBrewing && !isGrinding && !isMacerating; // suché drcení a louhování nepotřebují oheň pod kotlem

    // Trituratio (suché drcení) a Maceratio (louhování) nemají bublající var
    const bubbles = (isBrewing && !isGrinding && !isMacerating) ? `
    ...
    // Maceratio: klidné soustředné vlnky na hladině — pasivní louhování, žádný oheň ani bublání
    const ripples = isMacerating ? `
      <ellipse cx="90" cy="71" rx="8" ry="2.6" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="1">
        <animate attributeName="rx" values="8;38;8" dur="2.6s" repeatCount="indefinite"/>
        <animate attributeName="ry" values="2.6;12;2.6" dur="2.6s" repeatCount="indefinite"/>
        <animate attributeName="opacity" values="0.55;0;0.55" dur="2.6s" repeatCount="indefinite"/>
      </ellipse>
      <ellipse cx="90" cy="71" rx="8" ry="2.6" fill="none" stroke="rgba(255,255,255,0.24)" stroke-width="0.8">
        <animate attributeName="rx" values="8;34;8" dur="2.6s" repeatCount="indefinite" begin="0.9s"/>
        <animate attributeName="ry" values="2.6;11;2.6" dur="2.6s" repeatCount="indefinite" begin="0.9s"/>
        <animate attributeName="opacity" values="0.45;0;0.45" dur="2.6s" repeatCount="indefinite" begin="0.9s"/>
      </ellipse>
      <ellipse cx="90" cy="71" rx="8" ry="2.6" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="0.7">
        <animate attributeName="rx" values="8;30;8" dur="2.6s" repeatCount="indefinite" begin="1.7s"/>
        <animate attributeName="ry" values="2.6;9.5;2.6" dur="2.6s" repeatCount="indefinite" begin="1.7s"/>
        <animate attributeName="opacity" values="0.35;0;0.35" dur="2.6s" repeatCount="indefinite" begin="1.7s"/>
      </ellipse>` : '';
    ...
    return `<svg viewBox="0 0 180 150" width="180" height="150" xmlns="http://www.w3.org/2000/svg"
      ...
      <!-- vlnky na hladině (jen za varu, mimo Maceratio — to má vlastní klidné vlnky níž) -->
      ${isBrewing && !isMacerating ? `<ellipse cx="90" cy="71" rx="24" ry="7.5" fill="none" stroke="rgba(255,255,255,0.22)" stroke-width="1">
